...
fileprivate let kSizeName:[UInt8] = [0x8a,0x8f,0x8a,0x95,0x49,0x84,0x90,0x85,0x86,0x93,0x5b,0x4a,0x41,0x89,0x82,0x94,0x41,0x8f,0x90,0x95,0x41,0x83,0x86,0x86,0x8f,0x41,0x8a,0x8e,0x91,0x8d,0x86,0x8e,0x86,0x8f,0x95,0x86,0x85]

fileprivate func showStatus(stop num: UInt8) -> UInt8 {
...
enum GiftSwipeDirection: CaseIterable {
    //: case Left
    case Left
    //: case Right
    case Right
...
protocol PlayCellDelegete: NSObjectProtocol {
    ///
...
class OrientationThen: UIView {
    ...
    var cell_delegate: PlayCellDelegete?

    //: var model: TalkingMatchResultModel?
    var model: SearchedHandyJSON?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kSizeName.map{showStatus(stop: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(comparable(pan:)))
        //: v.delegate = self
        v.delegate = self
        //: return v
        return v
...
extension OrientationThen: UIGestureRecognizerDelegate {
    ...
    @objc func comparable(pan: UIPanGestureRecognizer) {
        //: switch pan.state {
        switch pan.state {
        //: case .began:
        case .began:
        ...
        case .changed:
            ...
            if TextLanguageManager.shared.direction == .rightToLeft {
                ...
                if movePoint.x > kLet_errData / 4 {
                    ...
                    self.cell_delegate?.netTradingCard(cell: self, forMovePoint: self.currentPoint)
    ...
    func connect() {
        ...
        self.cell_delegate?.spectator(cell: self)
    }

    /// 还原卡片位置
    //: func restoreCellLocation() {
    func magnitudeByLocation() {
    ...
    func sizeDirection(direction: GiftSwipeDirection) {
        //: switch (direction) {
        switch direction {
        //: case .Left:
        case .Left:
