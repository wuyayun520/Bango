...
/*: "position.x" :*/
fileprivate let k_valueName:String = "posframei"

/*: "slideIn" :*/
fileprivate let kSourceValue:[Character] = ["s","l","i","d","e","I","n"]

/*: "slideOut" :*/
fileprivate let kSufficientName:[Character] = ["s","l","i","d","e","O","u","t"]
...
// __CLOSE_PRINT__
//
//  VideoAppReactiveCompatible.swift
//  VertexKit
//
...
class VideoAppReactiveCompatible: NSObject {
    //: static let shared = GiftFloatViewManager()
    static let shared = VideoAppReactiveCompatible()
    //: private var giftArr = [SocialBroadcastModel]()
    private var giftArr = [RowModelType]()
    ...
    private lazy var giftView: EconomicAidFloatView = {
        //: let v = GiftFloatView()
        let v = EconomicAidFloatView()
        //: NameMacroDefine.getWindow().addSubview(v)
        NameMacroDefine.playWindow().addSubview(v)
        //: return v
        return v
        //: }()
    }()
...
extension VideoAppReactiveCompatible {
    ...
    private func equalFrom(_ model: RowModelType) {
        ...
        let isRTL = TextLanguageManager.shared.direction == .rightToLeft
        // 起始位置：屏幕外
        //: let startCenterX = isRTL ? -centerX : ScreenWidth + centerX
        let startCenterX = isRTL ? -centerX : kLet_errData + centerX
        // 目标位置：屏幕中间
        //: let targetCenterX = ScreenWidth / 2
        let targetCenterX = kLet_errData / 2
        // 结束位置：屏幕另一侧
        //: let endCenterX = isRTL ? ScreenWidth + centerX : -centerX
        let endCenterX = isRTL ? kLet_errData + centerX : -centerX
        ...
        let slideIn = CABasicAnimation(keyPath: (k_valueName.replacingOccurrences(of: "frame", with: "it") + "on.x"))
        ...
        self.giftView.layer.add(slideIn, forKey: (String(kSourceValue)))
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
            ...
            let slideOut = CABasicAnimation(keyPath: (k_valueName.replacingOccurrences(of: "frame", with: "it") + "on.x"))
            ...
            self.giftView.layer.add(slideOut, forKey: (String(kSufficientName)))
...
extension VideoAppReactiveCompatible {
    ...
    func equalModel() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.recievedWithName(noti:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.pushBackground),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.upForeground),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.nameAdd),
    ...
    @objc private func recievedWithName(noti: Notification) {
        ...
        guard let data = noti.object,
              ...
              let gift = JSONDeserializer<RowModelType>.deserializeFrom(dict: data as? [String: Any], designatedPath: nil),
              //: gift.broadType != BroadcastType.Unknow.rawValue
              gift.broadType != PickNameConvertible.Unknow.rawValue
        //: else {
        else {
            //: return
            return
        }
        //: let currUserType = RowReactiveCompatible.share.appUserConfigMode.userType
        let currUserType = RowReactiveCompatible.share.appUserConfigMode.userType
        // 过滤接口异常
        ...
        // 非特殊用户 不展示 不同地区用户礼物
        //: if currUserType != 0, gift.userCountryType != RowReactiveCompatible.share.appUserConfigMode.userCountryType {
        if currUserType != 0, gift.userCountryType != RowReactiveCompatible.share.appUserConfigMode.userCountryType {
            //: return
            return
        }
        ...
        self.fileCell(gift: gift)
    }

    /// 应用退到后台
    //: @objc private func enterBackground() {
    @objc private func pushBackground() {
    ...
    @objc private func upForeground() {
        //: self.activeWhenShow()
        self.appShow()
    }
