...
fileprivate let kTempValue:[UInt8] = [0x46,0x41,0x46,0x5b,0x7,0x4c,0x40,0x4b,0x4a,0x5d,0x15,0x6,0xf,0x47,0x4e,0x5c,0xf,0x41,0x40,0x5b,0xf,0x4d,0x4a,0x4a,0x41,0xf,0x46,0x42,0x5f,0x43,0x4a,0x42,0x4a,0x41,0x5b,0x4a,0x4b]

private func ifPlayer(column num: UInt8) -> UInt8 {
    return num ^ 47
}

/*: "JXBannerCellID" :*/
fileprivate let kSharedTitle:String = "log view view user colorJXBa"
fileprivate let kMakeData:[Character] = ["l","l","I","D"]

/*: "#FFFFFF" :*/
fileprivate let k_objectData:String = "add info medium gesture self#FFFFFF"

/*: "#BDB8B5" :*/
fileprivate let k_contentName:String = "#BDB8B5button transaction icon name"
...
protocol DataViewProtocol: NSObjectProtocol {
    // 加载更多数据
...
class KeyReusableView: UICollectionReusableView {
    //: weak var delegate: SocialPopularListFooterViewProtocol?
    weak var delegate: DataViewProtocol?
    //: var listData: [PremiumStarListModel]?
    var listData: [DirectoryListModel]? // 巨星计划数据
    //: var adBannderListData: [SocialAdBannerModel]?
    var adBannderListData: [EventMeasurable]? // 广告banner数据

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: kTempValue.map{ifPlayer(column: $0)}, encoding: .utf8)!)
    }
...
extension KeyReusableView {
    /// 获取当前视图高度
    /// - Returns: 高度
    //: func getFooterViewHeight() -> CGFloat {
    func footerSelect() -> CGFloat {
    ...
    func titleOpen(data: [DirectoryListModel]) {
        //: self.listData = data
        self.listData = data
    }
    ...
    func brainData(data: [EventMeasurable]) {
...
extension KeyReusableView: JXBannerDataSource, JXBannerDelegate {
    //: func jxBanner(_ banner: JXBannerType) -> (JXBannerCellRegister) {
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        //: return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: "JXBannerCellID")
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(kSharedTitle.suffix(4)) + "nnerCe" + String(kMakeData)))
    }
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(k_objectData.suffix(7))))!
        //: pageControl.inactiveSize = CGSize(width: 6, height: 6)
        pageControl.inactiveSize = CGSize(width: 6, height: 6)
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(k_contentName.prefix(7))))!
...
extension KeyReusableView {
    ...
    private func streamUp() {
        ...
        bannerView.snp.makeConstraints { make in
            ...
            make.size.equalTo(CGSize(width: kLet_errData - 30, height: 84))
        }
    }
}
