...
fileprivate let k_indexData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Welcome to my room" :*/
fileprivate let kTouchValue:String = "Welcomline model let time guard"
fileprivate let kButtonName:[Character] = ["e"," ","t","o"," ","m","y"," ","r","o","o","m"]

/*: "%@'s room" :*/
fileprivate let kNumberAgeValue:String = "%@\'"
fileprivate let kControlValue:[Character] = ["s"," ","r","o","o","m"]

/*: "CCCCCC" :*/
fileprivate let kResultData:[Character] = ["C","C","C","C","C","C"]

/*: "#E1D0FC" :*/
fileprivate let kGiftTitle:[Character] = ["#","E","1","D","0","F","C"]

/*: "#FFE3E5" :*/
fileprivate let kQuickValue:[Character] = ["#","F","F","E","3","E","5"]

/*: "get json error" :*/
fileprivate let kSizeValue:String = "video"
fileprivate let kItemContent:String = "equal make gameet j"
...
class ChemicalElementThen: UITableViewCell {
	var chemicalElementText: String = "slide"
	var bagArray: [AnyHashable] = []
	var daisyChainTitle: String = "request"
	var majorityArray: [AnyHashable] = []
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_indexData.reversed(), encoding: .utf8)!)
    }
    ...
    var listModel: EventTransformable? {
        ...
        didSet {
            ...
            //: if String(listModel.uid) == RowReactiveCompatible.share.loginUid {
            if String(listModel.uid) == RowReactiveCompatible.share.loginUid {
                ...
                countryLab.text = (String(kTouchValue.prefix(6)) + String(kButtonName)).localized
                //: countryLab.textColor = .appValueColor()
                countryLab.textColor = .appPath()
                //: countryLab.font = UIFont.pingfangFont(type: .Medium, fontSize: 15)
                countryLab.font = UIFont.ofAndSize(type: .Medium, fontSize: 15)
                ...
                countryLab.textColor = .side()
                //: countryLab.font = UIFont.pingfangFont(type: .Regular, fontSize: 15)
                countryLab.font = UIFont.ofAndSize(type: .Regular, fontSize: 15)
                //: countryLab.snp.updateConstraints { make in
                countryLab.snp.updateConstraints { make in
            ...
            headPicImgV.followOut(urlStr: listModel.pic)
            //: nameLab.text = "%@'s room".localizedArguments(listModel.name)
            nameLab.text = (kNumberAgeValue + String(kControlValue)).roundName(listModel.name)
            //: countryImgView.setUrlImage(urlStr: listModel.flag)
            countryImgView.followOut(urlStr: listModel.flag)
            //: hotLab.text = "\(listModel.score)"
            hotLab.text = "\(listModel.score)"

            //: let hotWidth  = hotLab.sizeThatFits(CGSize(width: ScreenWidth, height: 20)).width
            let hotWidth = hotLab.sizeThatFits(CGSize(width: kLet_errData, height: 20)).width
    ...
    private lazy var bgView: UIView = {
        ...
        v.layer.shadowColor = UIColor(hex: (String(kResultData)))?.withAlphaComponent(0.5).cgColor
    ...
    private lazy var bgvlayer: CAGradientLayer = {
        ...
        layer.frame = CGRect(x: 0, y: 0, width: kLet_errData - 24, height: 76)
        //: layer.startPoint = CGPoint(x: 0, y: 0)
        layer.startPoint = CGPoint(x: 0, y: 0)
        //: layer.endPoint = CGPoint(x: 1, y: 0.5)
        layer.endPoint = CGPoint(x: 1, y: 0.5)
        //: layer.colors = [UIColor(hex: "#E1D0FC")?.cgColor, UIColor(hex: "#FFE3E5")?.cgColor]
        layer.colors = [UIColor(hex: (String(kGiftTitle)))?.cgColor, UIColor(hex: (String(kQuickValue)))?.cgColor]
    ...
    private lazy var liveSvga: SVGAPlayer = {
        ...
        do {
            ...
            let url = DataEnvironmentReactiveCompatible.default.atType(type: .Party_List)
            ...
            printLog(message: (kSizeValue.replacingOccurrences(of: "video", with: "g") + String(kItemContent.suffix(4)) + "son error"))
        }
    ...
    private lazy var hotLab: UILabel = {
        ...
        lab.font = UIFont.ofAndSize(type: .Medium, fontSize: 13)
...
extension ChemicalElementThen {
    // 添加视图
    //: private func setupSubviews() {
    private func equalContent() {
