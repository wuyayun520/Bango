...
fileprivate let kTitleText:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_match_shadow" :*/
fileprivate let k_normalContent:String = "img_mat actual self i"
fileprivate let kAgeName:[Character] = ["a","d","o","w"]

/*: "icon_home_girl" :*/
fileprivate let kWillIndexData:String = "in self for add sectionicon_h"
fileprivate let k_termsUserTitle:String = "omreason"
fileprivate let k_equalChannelName:String = "to let_girl"

/*: "icon_home_v" :*/
fileprivate let k_keyContent:String = "manager view end resulticon_"

/*: "btn_dynamic_stop_nor" :*/
fileprivate let kEraseData:[Character] = ["b","t","n","_","d","y","n","a","m","i","c","_","s","t","o","p","_","n","o","r"]

/*: "#2DF2FF" :*/
fileprivate let k_ofText:String = "#"
fileprivate let k_colorData:String = "2DF2FFnumber to touch to"

/*: "· :*/
fileprivate let k_intimateData:[Character] = ["·"]

/*: "img_home_shadow" :*/
fileprivate let kCenterData:String = "img_hcontent self user gift view"
fileprivate let kTotalValue:String = "app table object typeome_"
fileprivate let kOfName:[Character] = ["s","h","a","d","o","w"]

/*: "icon_home_boy" :*/
fileprivate let k_frameLeadingValue:String = "self else text pushicon_"
fileprivate let k_giftNameValue:String = "random equal insert if pathhome_boy"

/*: "MetadataView" :*/
fileprivate let k_clickValue:String = "TUsetrue plus"
fileprivate let k_makeModelCollectionData:String = "ilHeadlog let interaction app"
fileprivate let kSizeName:String = "if array crushmCell"

/*: "#FFFFFF" :*/
fileprivate let kWillName:[Character] = ["#","F","F","F","F","F","F"]
...
class WithMakeBannerDelegate: UIView {
    ...
    private var model = SearchedHandyJSON()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kTitleText.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var gradientShadowImageView: UIImageView = {
        ...
        imageView.image = UIImage.collectionName(name: (String(k_normalContent.prefix(5)) + "atch_sh" + String(kAgeName)))
        //: return imageView
        return imageView
        //: }()
    }()
    ...
    private lazy var sexImgV: UIImageView = {
        //: let img = UIImageView()
        let img = UIImageView()
        //: img.image = UIImage.collectionName(name: "icon_home_girl")
        img.image = UIImage.collectionName(name: (String(kWillIndexData.suffix(6)) + k_termsUserTitle.replacingOccurrences(of: "reason", with: "e") + String(k_equalChannelName.suffix(5))))
    ...
    private lazy var authImgV: UIImageView = {
        ...
        img.image = UIImage.collectionName(name: (String(k_keyContent.suffix(5)) + "home_v"))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: return img
        return img
    ...
    private lazy var ageLab: UILabel = {
        ...
        lab.font = UIFont.ofAndSize(type: .Medium, fontSize: 11)
        //: lab.textColor = .white
        lab.textColor = .white
        //: return lab
        return lab
    ...
    lazy var playImageView: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.collectionName(name: (String(kEraseData))))
        //: imag.isHidden = true
        imag.isHidden = true
        //: return imag
        return imag
...
//        v.roundBgColor = UIColor.white
        ...
        v.progressColor = UIColor(hex: (k_ofText.capitalized + String(k_colorData.prefix(6))))!
        //: return v
        return v
        //: }()
    }()
...
extension WithMakeBannerDelegate {
    /// 更新视图UI
    /// - Parameter model: 数据模型
    //: func updatePicturesView(model: TalkingMatchResultModel) {
    func picturesMeanSunViewModify(model: SearchedHandyJSON) {
        //: self.model = model
        self.model = model
        ...
        ageLab.text = "·\(model.age)"
        //: userImg.setUrlImage(urlStr: model.headPic, placeImg: UIImage.collectionName(name: "img_home_shadow"))
        userImg.stopDown(urlStr: model.headPic, placeImg: UIImage.collectionName(name: (String(kCenterData.prefix(5)) + String(kTotalValue.suffix(4)) + String(kOfName))))
        //: if model.sex == Gender.male.rawValue {
        if model.sex == AppTextProtocol.male.rawValue {
            //: sexImgV.image = UIImage.collectionName(name: "icon_home_boy")
            sexImgV.image = UIImage.collectionName(name: (String(k_frameLeadingValue.suffix(5)) + String(k_giftNameValue.suffix(8))))
            //: } else {
        } else {
            //: sexImgV.image = UIImage.collectionName(name: "icon_home_girl")
            sexImgV.image = UIImage.collectionName(name: (String(kWillIndexData.suffix(6)) + k_termsUserTitle.replacingOccurrences(of: "reason", with: "e") + String(k_equalChannelName.suffix(5))))
        }
        ...
        self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
            ...
            self.progressView.reload(totalTime: self.MatchResultProgress_Timer, currTime: time)
            //: time -= 1
            time -= 1
            //: }, repeats: true)
        }, repeats: true)
...
extension WithMakeBannerDelegate: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: MetadataView.self, reuseIdentifier: (String(k_clickValue.prefix(4)) + "rDeta" + String(k_makeModelCollectionData.prefix(6)) + "erIte" + String(kSizeName.suffix(5))))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        if index == 0 && self.model.videoUrl.count > 1 { // 视频
            ...
            bannerCell.half(videoUrl: self.model.videoUrl, imgUrl: self.model.cover)
            /// 自动播放完视频后滑动到下一个
...
//                }
            ...
            bannerCell.bgImgView.stopDown(urlStr: str, placeImg: UIImage.cookie())
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(kWillName)))!.withAlphaComponent(1.0)
        //: pageControl.inactiveColor = UIColor(hex: "#FFFFFF")!.withAlphaComponent(0.6)
        pageControl.inactiveColor = UIColor(hex: (String(kWillName)))!.withAlphaComponent(0.6)
...
extension WithMakeBannerDelegate: JXBannerDelegate {
