...
fileprivate let k_addValue:[UInt8] = [0x79,0x72,0x74,0x65,0x72,0x20,0x64,0x6e,0x61,0x20,0x74,0x69,0x78,0x65,0x20,0x65,0x73,0x61,0x65,0x6c,0x70,0x20,0x2c,0x64,0x65,0x67,0x6e,0x61,0x68,0x63,0x20,0x73,0x75,0x74,0x61,0x74,0x53]

/*: "GET" :*/
fileprivate let kBoyDataName:[Character] = ["G","E","T"]
...
class ColorServicePlayTool: NSObject {
    //: var player: AVAudioPlayer?
    var player: AVAudioPlayer?
    //: open weak var delegate: TalkingAudioPlayToolDelegate?
    open weak var delegate: PanelThen?

    // singleton
    //: static let shared = TalkingAudioPlayTool()
    static let shared = ColorServicePlayTool()
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(imageInsert(notification:)), name: AVAudioSession.interruptionNotification, object: nil)
    }

    //: func play(urlStr: String) {
    func front(urlStr: String) {
        ...
        everyCompletion(audioUrl: urlStr) { data in
            //: do {
            do {
                ...
                self.appearShow(showMsg: String(bytes: k_addValue.reversed(), encoding: .utf8)!.localized)
                //: self.delegate?.playerStatusChange(status: .PlayerFinish)
                self.delegate?.chessPlayerView(status: .PlayerFinish)
...
extension ColorServicePlayTool {
    //: func downloadAudioWith(audioUrl: String, completion: @escaping AudioCompletionblock) {
    func everyCompletion(audioUrl: String, completion: @escaping AudioCompletionblock) {
        //: var urlOk = true
        var urlOk = true
        ...
        request.httpMethod = (String(kBoyDataName))
...
extension ColorServicePlayTool: AVAudioPlayerDelegate {
    /// 播放过程中中断
    //: @objc func interruptionChange(notification: NSNotification) -> Void {
    @objc func imageInsert(notification: NSNotification) {
        ...
        if changeReason?.intValue ?? 0 == AVAudioSession.InterruptionType.began.rawValue {
            ...
            self.delegate?.chessPlayerView(status: .PlayerInterruption)
        }
    }
