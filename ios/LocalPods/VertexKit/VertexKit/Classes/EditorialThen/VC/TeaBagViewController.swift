...
/*: "Face verification" :*/
fileprivate let kFirstValue:String = "Facein lab false"
fileprivate let k_contextReloadValue:[Character] = ["a","t","i","o","n"]
...
class TeaBagViewController: RecordReactiveCompatible {
	var viewNumber: Int = 38
	var pushCount: Double = -76.8
    ...
    override func viewDidLoad() {
        ...
        title = (String(kFirstValue.prefix(4)) + " verific" + String(k_contextReloadValue)).localized
        //: self.createUI()
        self.root()
    ...
    lazy var mainView: PopReactiveCompatible = {
        //: let view = TalkingVerificationExampleView()
        let view = PopReactiveCompatible()
        //: return view
        return view
        //: }()
    }()
...
extension TeaBagViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    //: private func takePhoto() {
    private func videoConnect() {
        ...
        self.dismiss(animated: true) {
            ...
            if let photo = info[UIImagePickerController.InfoKey.originalImage] as? UIImage {
                ...
                if self.isRegisterPush {
                    ...
                    let vc = FollowPhotoVc(photoImage: photo)
...
extension TeaBagViewController {
    //: func createUI() {
    func root() {
        ...
        mainView.btnClickBlock = { [weak self] () in
            ...
            AuthorizationReactiveCompatible.keep(true) { isOpen in
