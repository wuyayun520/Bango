...
fileprivate let k_frameStackData:[UInt8] = [0x50,0x57,0x50,0x4d,0x11,0x5a,0x56,0x5d,0x5c,0x4b,0x3,0x10,0x19,0x51,0x58,0x4a,0x19,0x57,0x56,0x4d,0x19,0x5b,0x5c,0x5c,0x57,0x19,0x50,0x54,0x49,0x55,0x5c,0x54,0x5c,0x57,0x4d,0x5c,0x5d]

private func semiprofessionalLeading(equal num: UInt8) -> UInt8 {
    return num ^ 57
}

/*: "icon_userdetail_header_shadow" :*/
fileprivate let k_matchData:[Character] = ["i","c"]
fileprivate let kLiveData:String = "on_useas height else up"
fileprivate let kRestoreValue:String = "model value blockail_"
fileprivate let kDetailText:String = "match random equal clearer_shadow"

/*: "icon_lounge" :*/
fileprivate let k_sName:String = "IC"
fileprivate let k_bagData:String = "on_looff var image"

/*: "Official" :*/
fileprivate let kFormTitle:String = "Officiself for"
fileprivate let k_indicatorValue:String = "margin"

/*: "Reviewing" :*/
fileprivate let k_voiceValue:String = "Revstring add title to push"
fileprivate let kTextValue:String = "ielistng"

/*: "#FF506D" :*/
fileprivate let kShowData:String = "image extension#FF50"
fileprivate let kSectionTitle:[Character] = ["6","D"]

/*: "icon_user_detail_intimate_header" :*/
fileprivate let kLocationText:[UInt8] = [0xea,0xe0,0xec,0xed,0xdc,0xf6,0xf0,0xe6,0xf1,0xdc,0xe7,0xe6,0xf7,0xe2,0xea,0xef,0xdc,0xea,0xed,0xf7,0xea,0xee,0xe2,0xf7,0xe6,0xdc,0xeb,0xe6,0xe2,0xe7,0xe6,0xf1]

private func kitLayer(frame num: UInt8) -> UInt8 {
    return num ^ 131
}

/*: "?toUid= :*/
fileprivate let kViewData:String = "half view photo error index?toUid="

/*: "MetadataView" :*/
fileprivate let k_phoneSelectValue:[Character] = ["T","U","s","e","r","D","e","t","a","i","l","H","e","a","d","e","r","I","t","e","m"]
fileprivate let kLabelErrContent:String = "make cell extension number viewCell"

/*: "#FFFFFF" :*/
fileprivate let k_makeGameText:String = "#FFFFFFcamera center app model text"

/*: "#BDB8B5" :*/
fileprivate let kShowAddTitle:[Character] = ["#","B","D","B","8","B","5"]

/*: "head_detail_bottom" :*/
fileprivate let k_originName:String = "arrayead"
fileprivate let kValueLabData:String = "ail_bre self false direction"

/*: "#2DF2FF" :*/
fileprivate let k_viewValue:String = "#2DF2FFfollow drop"

/*: "Live" :*/
fileprivate let k_stopName:[Character] = ["L","i","v","e"]

/*: "#48FF1E" :*/
fileprivate let kMenuModelName:String = "#48FF1Etable tool"

/*: "Online" :*/
fileprivate let kContentValue:String = "top right request assertOnline"

/*: "#FF4B7F" :*/
fileprivate let k_showData:String = "#FF4Btype add make list manager"
fileprivate let kFromData:[Character] = ["7","F"]

/*: "busy" :*/
fileprivate let kDeviceFileTitle:String = "BUSY"
...
let kLet_screenStatusValue = (302.0 + kLet_topData)

//: class TalkingUserDetailHeaderView: UIView {
class ItemBannerDelegate: UIView {
    //: var VideoCallBlock: (() -> Void)?
    var VideoCallBlock: (() -> Void)?
    //: private var userInfoModel = TalkingUserInfoModel()
    private var userInfoModel = EnvironmentHandyJSON()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_frameStackData.map{semiprofessionalLeading(equal: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerView: JXBanner = {
        ...
        let frame = CGRect(x: 0, y: 0, width: kLet_errData, height: kLet_screenStatusValue)
        ...
        view.placeholderImgView.image = UIImage.even()
    ...
    private lazy var shadowBgView: UIImageView = {
        //: let v = UIImageView(image: UIImage.collectionName(name: "icon_userdetail_header_shadow"))
        let v = UIImageView(image: UIImage.collectionName(name: (String(k_matchData) + String(kLiveData.prefix(6)) + "rdet" + String(kRestoreValue.suffix(4)) + "head" + String(kDetailText.suffix(9)))))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var vipBgView: UIImageView = {
        ...
        v.frame = CGRect(x: 0, y: kLet_screenStatusValue - 15, width: kLet_errData, height: 15)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.collectionName(name: (k_sName.lowercased() + String(k_bagData.prefix(5)) + "unge"))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var officalImgV: UIButton = {
        ...
        img.australianState(color: UIColor.nameTheory(), forState: .normal)
        //: img.setTitle("Official".localized, for: .normal)
        img.setTitle((String(kFormTitle.prefix(6)) + k_indicatorValue.replacingOccurrences(of: "margin", with: "al")).localized, for: .normal)
    ...
    private lazy var onlineImgV: AddReactiveCompatible = {
    ...
    private lazy var liveView: RecordMiniView = {
        //: let v = UserLivePullStreamsView()
        let v = RecordMiniView()
        //: v.isHidden = true
        v.isHidden = true
        //: v.playEndBlock = {
        v.playEndBlock = {
        ...
        v.tapGestureBlock = { [weak self] in
            ...
            guard let self = self else { return }
            //: MiraclePushManager.share.func_audiencePushToLiveRoomVC(uid: self.userInfoModel.uid, enterType: .userDetail)
            MiraclePushManager.share.equalButtonKindTo(uid: self.userInfoModel.uid, enterType: .userDetail)
        }
        //: return v
        return v
        //: }()
    ...
    private lazy var reViewingLB: UILabel = {
        ...
        lb.text = (String(k_voiceValue.prefix(3)) + kTextValue.replacingOccurrences(of: "list", with: "wi")).localized
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.backgroundColor = UIColor.init(hex: "#FF506D")
        lb.backgroundColor = UIColor(hex: (String(kShowData.suffix(5)) + String(kSectionTitle)))
    ...
    lazy var lastLoginLab: UILabel = {
        ...
        lb.font = .ofAndSize(type: .Medium, fontSize: 12)
        //: lb.textColor = .white
        lb.textColor = .white
        //: return lb
        return lb
    ...
    private lazy var intimateBtn: AddReactiveCompatible = {
        //: let btn = TalkingButton()
        let btn = AddReactiveCompatible()
        //: btn.setImage(UIImage.collectionName(name: "icon_user_detail_intimate_header"), for: .normal)
        btn.setImage(UIImage.collectionName(name: String(bytes: kLocationText.map{kitLayer(frame: $0)}, encoding: .utf8)!), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
...
extension ItemBannerDelegate {
    /// 查看视频
    //: private func goToVideo() {
    private func ask() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: self.userInfoModel.videoUrl)
        let vc = TalkingVideoVc(videoPath: self.userInfoModel.videoUrl)
        //: if RowReactiveCompatible.share.loginUid != self.userInfoModel.uid {
        if RowReactiveCompatible.share.loginUid != self.userInfoModel.uid {
    ...
    private func digitizer(_ index: Int) {
        ...
        let vc = RowFlowLayout(momentModel: currenModel, index: index, type: .normal)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.belowResume()?.navigationController?.pushViewController(vc, animated: true)
    }
    ...
    @objc func portion() {
        ...
        let url = AstatineReactiveCompatible.typeOn(urlType: .Intimate) + (String(kViewData.suffix(7))) + "\(self.userInfoModel.uid)"
...
extension ItemBannerDelegate: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: MetadataView.self, reuseIdentifier: (String(k_phoneSelectValue) + String(kLabelErrContent.suffix(4))))
    }

    //: func jxBanner(_ banner: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        //: let bannerCell = cell as! TUserDetailHeaderItemCell
        let bannerCell = cell as! MetadataView
        //: let urlStr = self.dataArray[index]
        let urlStr = self.dataArray[index]

        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == AppTextProtocol.female.rawValue && userInfoModel.videoUrl.count > 1 { // 视频
            //: bannerCell.setVideoView()
            bannerCell.familyTitleComment()
            //: bannerCell.configModel(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            bannerCell.half(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            ...
            bannerCell.auditoryImage()
            //: bannerCell.bgImgView.setUrlImage(urlStr: urlStr, placeImg: UIImage.placeSquareBigImgBanner())
            bannerCell.bgImgView.stopDown(urlStr: urlStr, placeImg: UIImage.cookie())
        }
    ...
    func jxBanner(_: JXBannerType, centerIndex: Int, centerCell _: UICollectionViewCell) {
        //: guard userInfoModel.uid == RowReactiveCompatible.share.loginUid else {
        guard userInfoModel.uid == RowReactiveCompatible.share.loginUid else {
            //: return
            return
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(k_makeGameText.prefix(7))))!
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(kShowAddTitle)))!
...
extension ItemBannerDelegate: JXBannerDelegate {
    ...
    public func jxBanner(_: JXBannerType, didSelectItemAt index: Int) {
        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == AppTextProtocol.female.rawValue && userInfoModel.videoUrl.count > 1 {
            //: goToVideo()
            ask()
            ...
            if userInfoModel.sex == AppTextProtocol.female.rawValue && userInfoModel.videoUrl.count > 1 && curren > 0 {
                //: curren -= 1
                curren -= 1
            }
            //: goToPhoto(curren)
...
extension ItemBannerDelegate {
    ...
    func weltanschauung(_ userModel: EnvironmentHandyJSON) {
        ...
        if userModel.sex == AppTextProtocol.female.rawValue && userModel.videoUrl.count > 1 {
        ...
        bannerView.reloadView()
        // vip
        //: let normalImg = UIImage.collectionName(name: "head_detail_bottom").withTintColor(.white)
        let normalImg = UIImage.collectionName(name: (k_originName.replacingOccurrences(of: "array", with: "h") + "_det" + String(kValueLabData.prefix(5)) + "ottom")).withTintColor(.white)
        //: if userModel.loungePlus {
        if userModel.loungePlus {
            //: if let vipModel = RowReactiveCompatible.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userModel.vipSkinId}).first {

            if let vipModel = RowReactiveCompatible.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userModel.vipSkinId }).first {
                //: vipBgView.setUrlImage(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
                vipBgView.stopDown(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
            }
            //: } else {
        } else {
        ...
        self.nicknameLab.textColor = userModel.loungePlus ? .coloured() : .white
        ...
        if userModel.intimateUrl.count > 0 {
            ...
            intimateImgV.followOut(urlStr: userModel.intimateUrl)
        }
        // 在线状态
        ...
        if userModel.livePlayUrl.count > 0 {
            ...
            let image = UIImage.thoughtImageShowSize(color: UIColor(hex: (String(k_viewValue.prefix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.circle(), for: .normal)
            onlineImgV.setImage(image.circleContext(), for: .normal)
            //: onlineImgV.setTitle("Live".localized, for: .normal)
            onlineImgV.setTitle((String(k_stopName)).localized, for: .normal)
            ...
            let image = UIImage.thoughtImageShowSize(color: UIColor(hex: (String(kMenuModelName.prefix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.circle(), for: .normal)
            onlineImgV.setImage(image.circleContext(), for: .normal)
            //: onlineImgV.setTitle("Online".localized, for: .normal)
            onlineImgV.setTitle((String(kContentValue.suffix(6))).localized, for: .normal)
            ...
            let image = UIImage.thoughtImageShowSize(color: UIColor(hex: (String(k_showData.prefix(5)) + String(kFromData)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.circle(), for: .normal)
            onlineImgV.setImage(image.circleContext(), for: .normal)
            //: onlineImgV.setTitle("busy".localized, for: .normal)
            onlineImgV.setTitle((kDeviceFileTitle.lowercased()).localized, for: .normal)
            //: } else {
        } else {
        ...
        // 直播
        //: if RowReactiveCompatible.share.loginUid != userModel.uid &&
        if RowReactiveCompatible.share.loginUid != userModel.uid,
        ...
        if liveView.isHidden == false {
            ...
            liveView.chemicalElement(liveUrl: userModel.livePlayUrl)
        }
    }

    /// 添加视图
    //: private func setupSubviews() {
    private func intervaleraction() {
    ...
    private func lab() {
        ...
        liveView.snp.makeConstraints { make in
            ...
            make.top.equalTo(kLet_topData + 80)
        ...
        reViewingLB.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(kLet_topData + 15)
