...
fileprivate let kModelValue:[UInt8] = [0xf5,0xf2,0xf5,0xe8,0xb4,0xff,0xf3,0xf8,0xf9,0xee,0xa6,0xb5,0xbc,0xf4,0xfd,0xef,0xbc,0xf2,0xf3,0xe8,0xbc,0xfe,0xf9,0xf9,0xf2,0xbc,0xf5,0xf1,0xec,0xf0,0xf9,0xf1,0xf9,0xf2,0xe8,0xf9,0xf8]

private func succeedCell(index num: UInt8) -> UInt8 {
    return num ^ 156
}
...
// __CLOSE_PRINT__
//
//  GiftHorseReactiveCompatible.swift
//  AbroadTalking
//
...
class GiftHorseReactiveCompatible: RecordReactiveCompatible {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: TitleDetailReactiveCompatible!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = TitleDetailReactiveCompatible(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: kModelValue.map{succeedCell(index: $0)}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class TitleDetailReactiveCompatible: NSObject {
	var bagOff: Bool = true
	var blockCount: Int = 37
	var videoQuantityContent: String = "block"
	var firstDictionary: [AnyHashable: String] = [:]
	var atQuantity: Double = -45.3
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: kLet_errData, height: kLet_nameValue * 0.6)
    ...
    func neglect(corners: UIRectCorner, radius: CGFloat) {
...
extension TitleDetailReactiveCompatible: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source _: UIViewController) -> UIPresentationController? {
        ...
        let res = SourcePresentationController(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension TitleDetailReactiveCompatible: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            makeRange(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            screenToContext(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func makeRange(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func screenToContext(transitionContext: UIViewControllerContextTransitioning) {
...
class SourcePresentationController: UIPresentationController, UIGestureRecognizerDelegate {
	var toQuantity: Int = 41
	var appearQuantity: Double = -35.2
	var friskingArray: [AnyHashable] = []
	var segmentDictionary: [AnyHashable: String] = [:]
	var mightEnable: Bool = false
	var titleMessageQuantity: Double = -17.5

    //: var transition: BasePresentTransition!
    var transition: TitleDetailReactiveCompatible!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: TitleDetailReactiveCompatible, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func offEqual() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(nameRecognizer))
        //: tap.delegate = self
        tap.delegate = self
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: kLet_errData, height: kLet_nameValue)
