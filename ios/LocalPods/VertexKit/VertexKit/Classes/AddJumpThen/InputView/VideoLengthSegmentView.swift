...
fileprivate let k_cellContent:[Character] = ["设","\u{7f6e}","普","\u{901a}","状","态","的","文","字","颜","色","\u{65f6}"," ","\u{8bf7}","使","用","R","G","B","空","间","的"]
fileprivate let kDealValue:String = "颜色值"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let kListName:[Character] = ["\u{8bbe}","置","选","中","状"]
fileprivate let kMakeContent:String = "\u{6001}的文字"
fileprivate let kTableData:String = "使用RGby if"

/*: "init(coder:) has not been implemented" :*/
fileprivate let kToData:[UInt8] = [0x2b,0x2c,0x2b,0x36,0x6a,0x21,0x2d,0x26,0x27,0x30,0x78,0x6b,0x62,0x2a,0x23,0x31,0x62,0x2c,0x2d,0x36,0x62,0x20,0x27,0x27,0x2c,0x62,0x2b,0x2f,0x32,0x2e,0x27,0x2f,0x27,0x2c,0x36,0x27,0x26]

/*: "设置的下标不合法!!" :*/
fileprivate let k_tapData:String = "设置的下标\u{4e0d}"
fileprivate let kPageContent:String = "合法!!"

/*: "Bags" :*/
fileprivate let kSocialName:String = "Bagslet range"
...
open class VideoLengthSegmentView: UIView {
	var streetwiseName: String = "change"
	var liveTitle: String = "name"

    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: SourceSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.exaggerateAccept(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(k_cellContent) + kDealValue.capitalized))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.exaggerateAccept(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(kListName) + kMakeContent + "颜\u{8272}时 \u{8bf7}" + String(kTableData.prefix(4)) + "B空间的颜色\u{503c}"))
        }
    ...
    fileprivate func exaggerateAccept(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: SourceSegmentStyle, titles: [String]) {
    ...
    @objc func drape(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? BeamThen else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kToData.map{$0^66}, encoding: .utf8)!)
    }
}
...
public extension VideoLengthSegmentView {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func examineedObject(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (k_tapData + kPageContent.capitalized))
    ...
    func exceptGo(_ titles: [String]) {
        ...
        examineedObject(0, animated: true)
    }
}
...
//: extension ScrollSegmentView {
extension VideoLengthSegmentView {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func messageContent() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = BeamThen(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.drape(_:)))
            ...
            if title == (String(kSocialName.prefix(4))).localized {
    ...
    fileprivate func modelUpwardly() {
        ...
        if let firstLabel = labelsArray[0] as? BeamThen {
    ...
    public func headerValue(_ currentIndex: Int) {
