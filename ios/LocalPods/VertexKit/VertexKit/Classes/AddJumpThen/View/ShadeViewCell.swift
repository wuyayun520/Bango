...
fileprivate let kOfViewValue:[UInt8] = [0x88,0x8f,0x88,0x95,0xc9,0x82,0x8e,0x85,0x84,0x93,0xdb,0xc8,0xc1,0x89,0x80,0x92,0xc1,0x8f,0x8e,0x95,0xc1,0x83,0x84,0x84,0x8f,0xc1,0x88,0x8c,0x91,0x8d,0x84,0x8c,0x84,0x8f,0x95,0x84,0x85]

private func letSelf(sub num: UInt8) -> UInt8 {
    return num ^ 225
}

/*: "#30D00B" :*/
fileprivate let kByUserContent:String = "#30D0table model name in"
fileprivate let k_ofName:String = "0Blet to description"

/*: "icon_receivedcalls" :*/
fileprivate let k_equalData:[Character] = ["i"]
fileprivate let k_labData:String = "con_rcell gift else to"
fileprivate let kVerticalData:String = "vedcallbond"

/*: "Received calls" :*/
fileprivate let kViewData:[Character] = ["R","e","c","e","i","v","e","d"," ","c","a"]
fileprivate let k_keyName:[Character] = ["l","l","s"]

/*: "icon_dialedcalls" :*/
fileprivate let k_partyName:[Character] = ["i","c","o","n","_"]
fileprivate let k_dataTitle:String = "viewialeview"

/*: "Dialed calls" :*/
fileprivate let kNameRawData:[Character] = ["D","i","a","l","e","d"," ","c","a","l"]
fileprivate let kAppContent:String = "lgift"

/*: "%02i:%02i" :*/
fileprivate let k_tableText:[Character] = ["%","0","2","i",":","%","0","2","i"]
...
class ShadeViewCell: UITableViewCell {
	var endSum: Int = 100
	var ofCount: Double = -85.1
	var chromaticColorArray: [AnyHashable] = []
	var gestureAtDictionary: [AnyHashable: String] = [:]
	var numerateSum: Int = 32
	var confinementNumber: Double = -45.7
	var videoArray: [AnyHashable] = []
	var colorDictionary: [AnyHashable: String] = [:]

    //: var currenModel = TalkingVideoCallRecordModel()
    var currenModel = AppearRecordModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kOfViewValue.map{letSelf(sub: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var durationLB: UILabel = {
        ...
        label.font = .ofAndSize(type: .Medium, fontSize: 10)
        //: label.textColor = .appValueDetailColor()
        label.textColor = .side()
    ...
    lazy var onlineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(kByUserContent.prefix(5)) + String(k_ofName.prefix(2))))
...
extension ShadeViewCell {
    //: func setVideoCallCell(model: TalkingVideoCallRecordModel) {
    func logBlock(model: AppearRecordModel) {
        //: currenModel = model
        currenModel = model
        ...
        if !model.headPicFrame.isEmptyString {
            ...
            iconBorderBtn.outWhen(urlStr: model.headPicFrame)
            //: } else {
        } else {
        ...
        if model.callStatus == 0 {
            //: rcordImg.image = UIImage.collectionName(name: "icon_receivedcalls")
            rcordImg.image = UIImage.collectionName(name: (String(k_equalData) + String(k_labData.prefix(5)) + "ecei" + kVerticalData.replacingOccurrences(of: "bond", with: "s")))
            //: rcordLB.text = "Received calls".localized
            rcordLB.text = (String(kViewData) + String(k_keyName)).localized
            //: } else {
        } else {
            //: rcordImg.image = UIImage.collectionName(name: "icon_dialedcalls")
            rcordImg.image = UIImage.collectionName(name: (String(k_partyName) + k_dataTitle.replacingOccurrences(of: "view", with: "d") + "calls"))
            //: rcordLB.text = "Dialed calls".localized
            rcordLB.text = (String(kNameRawData) + kAppContent.replacingOccurrences(of: "gift", with: "s")).localized
        }
