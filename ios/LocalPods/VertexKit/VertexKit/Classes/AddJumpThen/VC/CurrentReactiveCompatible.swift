...
fileprivate let kFrameData:[UInt8] = [0x53,0x58,0x53,0x5e,0x12,0x4d,0x59,0x4e,0x4f,0x5c,0x24,0x13,0xa,0x52,0x4b,0x5d,0xa,0x58,0x59,0x5e,0xa,0x4c,0x4f,0x4f,0x58,0xa,0x53,0x57,0x5a,0x56,0x4f,0x57,0x4f,0x58,0x5e,0x4f,0x4e]

fileprivate func viewSuper(data num: UInt8) -> UInt8 {
...
/*: "icon_photo_giftbg" :*/
fileprivate let kEventText:String = "icon_pcount view user send"
fileprivate let k_userData:String = "image"
fileprivate let kResignData:[Character] = ["o","t","o","_","g","i","f","t","b","g"]

/*: "Send" :*/
fileprivate let k_labData:String = "make check show manager varSend"

/*: "nav_back_black_nor" :*/
fileprivate let kToolName:[Character] = ["n","a","v","_","b","a","c","k","_","b"]
fileprivate let k_fieldValue:String = "lack_nortitle detail"

/*: "toUid" :*/
fileprivate let k_keyListData:String = "view whotoUid"

/*: "giftId" :*/
fileprivate let kColorContent:String = "giftIdhidden action"

/*: "giftNum" :*/
fileprivate let kResumeValue:String = "giftNummodel image live let"

/*: "unlockMsgId" :*/
fileprivate let kMakeSendName:[Character] = ["u","n","l","o","c"]
fileprivate let kEventName:String = "kMsgIdmake effect model"

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let k_textData:[UInt8] = [0x7c,0x93,0x93,0x92,0x4e,0x8f,0x4e,0x53,0x6e,0x4e,0x56,0x53,0x6e,0x4e,0x75,0x9d,0x9a,0x92,0x4e,0x91,0x9d,0x97,0x9c,0xa1,0x57,0x4e,0xa2,0x9d,0x4e,0xa3,0x9c,0x9a,0x9d,0x91,0x99]

fileprivate func rowTrueManager(equal num: UInt8) -> UInt8 {
...
/*: "icon_photo_lock" :*/
fileprivate let k_backgroundName:String = "livecon"
fileprivate let k_directionValue:String = "size self female else scaleo_lock"

/*: "Click on the screen to view 15s" :*/
fileprivate let k_imageDismissAcrossData:[UInt8] = [0xf7,0xd8,0xdd,0xd7,0xdf,0x94,0xdb,0xda,0x94,0xc0,0xdc,0xd1,0x94,0xc7,0xd7,0xc6,0xd1,0xd1,0xda,0x94,0xc0,0xdb,0x94,0xc2,0xdd,0xd1,0xc3,0x94,0x85,0x81,0xc7]

/*: "icon_photo_burn" :*/
fileprivate let k_mainText:String = "icon_pfalse view equal"
fileprivate let kTitleMakeText:String = "burcancel"

/*: "Already burned" :*/
fileprivate let k_equalName:String = "Alreaddata view"
...
//: typealias IntimatePhotoBlock = (_ model: TalkingAreaOutlookJsonModel) -> Void
typealias IntimatePhotoBlock = (_ model: TalkingAreaOutlookJsonModel) -> Void

//: class TalkingIntimatePhotoPreviewVC: TalkingBaseViewController {
class CurrentReactiveCompatible: RecordReactiveCompatible {
	var clickTotal: Int = 38
    ...
    //: private var imageTotalData = ClickCellData(direction: .MsgDirectionOutgoing)
    private var imageTotalData = ClickCellData(direction: .MsgDirectionOutgoing) // 图片消息数据
    //: private var photoModel = TalkingAreaOutlookJsonModel()
    private var photoModel = TalkingAreaOutlookJsonModel() // 当前私密照片模型
    ...
    ///   - imageCellData: 图片模型
    ///   - toUid: 对方Id
    //: init(imageCellData: ClickCellData, toUid: String) {
    init(imageCellData: ClickCellData, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (imageTotalData.msgModel.user.uid == Int(RowReactiveCompatible.share.loginUserMode.userID))
        isMySelf = (imageTotalData.msgModel.user.uid == Int(RowReactiveCompatible.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kFrameData.map{viewSuper(data: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.collectionName(name: (String(kEventText.prefix(6)) + k_userData.replacingOccurrences(of: "image", with: "h") + String(kResignData)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.followOut(urlStr: photoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(k_labData.suffix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(boneOfNameion), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.collectionName(name: (String(kToolName) + String(k_fieldValue.prefix(8)))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
        //: btn.addTarget(self, action: #selector(backButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(writerSName), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension CurrentReactiveCompatible {
    ...
    func coloring(giftId: String, giftNum: String) {
        ...
        params[(String(k_keyListData.suffix(5)))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(kColorContent.prefix(6)))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(kResumeValue.prefix(7)))] = giftNum
        //: params["unlockMsgId"] = self.imageTotalData.msgModel.msgInfo.msgId
        params[(String(kMakeSendName) + String(kEventName.prefix(6)))] = self.imageTotalData.msgModel.msgInfo.msgId
...
extension CurrentReactiveCompatible {
    ...
    @objc private func boneOfNameion() {
        //: req_sendGiftMsg(giftId: photoModel.giftId, giftNum: "1")
        coloring(giftId: photoModel.giftId, giftNum: "1")
    }

    /// 阅后即焚15s倒计时开始
    //: @objc private func burnPhotoTapGesture() {
    @objc private func nameOn() {
        ...
        self.infoCustomHandler(urlStr: photoModel.originUrl, isMosaic: false) { [weak self] isSucceed in
            //: ProgressHUD.dismiss()
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.reload(totalTime: 150, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension CurrentReactiveCompatible {
    ...
    private func cellUi() {
        ...
        if photoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.desLab.text = String(bytes: k_textData.map{rowTrueManager(equal: $0)}, encoding: .utf8)!.roundName(photoModel.giftName, photoModel.giftPrice)
            ...
            if photoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.collectionName(name: (k_backgroundName.replacingOccurrences(of: "live", with: "i") + "_phot" + String(k_directionValue.suffix(6))))
                //: self.desLab.text = "Click on the screen to view 15s".localized
                self.desLab.text = String(bytes: k_imageDismissAcrossData.map{$0^180}, encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnPhotoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(nameOn))
                ...
                self.intimateStatusImgV.image = UIImage.collectionName(name: (String(k_mainText.prefix(6)) + "hoto_" + kTitleMakeText.replacingOccurrences(of: "cancel", with: "n")))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(k_equalName.prefix(6)) + "y burned").localized
            }
        ...
        self.infoCustomHandler(urlStr: imgUrlStr, isMosaic: imgIsMosaic)
    }
    ...
    private func infoCustomHandler(urlStr: String, isMosaic: Bool, completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
