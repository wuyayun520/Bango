...
fileprivate let k_underTitle:[UInt8] = [0x4c,0x4b,0x4c,0x51,0xd,0x46,0x4a,0x41,0x40,0x57,0x1f,0xc,0x5,0x4d,0x44,0x56,0x5,0x4b,0x4a,0x51,0x5,0x47,0x40,0x40,0x4b,0x5,0x4c,0x48,0x55,0x49,0x40,0x48,0x40,0x4b,0x51,0x40,0x41]

private func nameSize(hidden num: UInt8) -> UInt8 {
    return num ^ 37
}

/*: "SHBDropMenuCell" :*/
fileprivate let k_sizeViewBackgroundText:[Character] = ["S","H","B","D","r","o","p","M","e","n","u","C"]
fileprivate let k_positionName:String = "iconl"

/*: "title" :*/
fileprivate let kStyleTitle:String = "ofiofle"
...
protocol SourceViewDelegate {
    ...
    func nameFor(index: Int, title: String)
}

//: public class DropDownMenuView: UIView {
public class DelayViewDelegate: UIView {
	var modelSocialCount: Double = 97.6
	var sightDictionary: [AnyHashable: String] = [:]
	var transactionQuantity: Double = 93.6
	var ageDictionary: [AnyHashable: String] = [:]
	var modelCount: Double = -85.7
	var pointDictionary: [AnyHashable: String] = [:]

    //: var menuDelegate: DropDownMenuViewDelegate?
    var menuDelegate: SourceViewDelegate?
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.PlonkDownRowUserConfig.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.MediaDelayJump.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.MediaDelayJump.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.MediaDelayJump.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? SourceMenuStyle.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
    ...
    public class func textByArray(anchorView: UIView, titleArray: [String], imageArray: [UIImage]) -> DelayViewDelegate {
        ...
        let menuView = ditheredColorArray(anchorView: anchorView, menuArray: menuArray)
        //: return menuView
        return menuView
    }

    //: public class func pullDropDrownMenu(anchorView: UIView, menuArray: [DropMenuModel]) -> DropDownMenuView {
    public class func ditheredColorArray(anchorView: UIView, menuArray: [SendMenuModel]) -> DelayViewDelegate {
        //: let window = NameMacroDefine.getWindow()
        let window = NameMacroDefine.playWindow()
        //: let menuView = DropDownMenuView()
        let menuView = DelayViewDelegate()
...
//        menuView.refreshUI()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_underTitle.map{nameSize(hidden: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func showMake() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func doImageParadigm() -> UITableView {
    ...
    private func largenessTouch() {
        //: addSubview(contentView)
        addSubview(contentView)
        //: mTable.register(DropMenuCell.self, forCellReuseIdentifier: "SHBDropMenuCell")
        mTable.register(MediaView.self, forCellReuseIdentifier: (String(k_sizeViewBackgroundText) + k_positionName.replacingOccurrences(of: "icon", with: "el")))
    ...
    private func orientationPointView() {
    ...
    private func magnitudeTriangle() {
		...
		//: if_extract_code "mixIn" begin
		
		var mixIn = false
		if !mixIn {
		    var isValue = false
		    if #available(iOS 13.0, *) {
		        isValue = contentView.editingInteractionConfiguration == .none
		    }
		    mixIn = isValue
		}
		
		//: if_extract_code "mixIn" end
    ...
    override public func touchesBegan(_: Set<UITouch>, with _: UIEvent?) {
        //: animateRemoveView()
        orientationPointView()
    }
}

//: extension DropDownMenuView: UITableViewDelegate, UITableViewDataSource {
extension DelayViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(k_sizeViewBackgroundText) + k_positionName.replacingOccurrences(of: "icon", with: "el")), for: indexPath) as! MediaView
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (kStyleTitle.replacingOccurrences(of: "of", with: "t")))
        //: animateRemoveView()
        orientationPointView()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.nameFor(index: indexPath.row, title: cellModel.title ?? "")
    }
}
