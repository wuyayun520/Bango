...
/*: "uid" :*/
fileprivate let kClickData:String = "UID"

/*: "onlineStatus" :*/
fileprivate let k_inputName:[Character] = ["o","n","l","i","n"]
fileprivate let k_voiceValue:String = "eStatusin let color"

/*: "isNewUser" :*/
fileprivate let k_sColorTitle:String = "isNewUsevideo equal equal"
fileprivate let k_sessionData:String = "bubble"

/*: "isTPAuth" :*/
fileprivate let k_centerContent:String = "title"
fileprivate let kAddData:String = "info view or functionsTPAuth"

/*: "totalIntimate" :*/
fileprivate let k_aConnectName:[Character] = ["t","o","t","a","l","I","n","t","i","m","a","t"]
fileprivate let k_edgeText:[Character] = ["e"]

/*: "userStatus" :*/
fileprivate let kTableArcText:String = "usecell"

/*: "intimate" :*/
fileprivate let k_priceName:String = "metimate"
...
class DelayListManager {
    ...
    private var moreMsgModel: PhaticCommunicationThen = //: return PhaticCommunicationThen.getPlaceholderModel(chatType: .moreMsg)
        .labType(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func resumeInitData() {
        //: self.officialArr = DelayConversationListener.shared.getOfficialList()
        self.officialArr = DelayConversationListener.shared.titleAdd()
        //: self.topArr = DelayConversationListener.shared.topConvList
        self.topArr = DelayConversationListener.shared.topConvList
        //: self.normalArr = DelayConversationListener.shared.norConvList
        self.normalArr = DelayConversationListener.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        digitiserLevelModelDataStatus()
    }

    // 请求聊天室
    //: func req_chatRommArr(completion: @escaping (_ succeed: Bool) -> Void) {
    func endCurrent(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: DelayConversationListener.shared.getChatRoomData { succeed in
        DelayConversationListener.shared.custom { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = DelayConversationListener.shared.chatRoomList
                self.chatRoomArr = DelayConversationListener.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func bruteUser(indexPath: IndexPath) -> PhaticCommunicationThen? {
    ...
    func alter(model: PhaticCommunicationThen?) -> PhaticCommunicationThen? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: FrontRecordCacheModel? = MusicInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: FrontRecordCacheModel? = MusicInfoManager.totalerval(targetId: targetId)
    ...
    func listLoad(conversationList: [PhaticCommunicationThen], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        //: MusicInfoManager.cache_getUserInfo(with: tempArr) { _, error in
        MusicInfoManager.iconTool(with: tempArr) { _, error in
    ...
    func conversationModel(conversationModel: PhaticCommunicationThen) {
    ...
    func searchedCompletion(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: DelayConversationListener.shared.func__getConversationList { arr, leftMore in
        DelayConversationListener.shared.deadlineFilter { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [PhaticCommunicationThen] = //: return Array<PhaticCommunicationThen>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [PhaticCommunicationThen] = //: return Array<PhaticCommunicationThen>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [PhaticCommunicationThen] = //: return Array<PhaticCommunicationThen>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [PhaticCommunicationThen] = //: return Array<PhaticCommunicationThen>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [PhaticCommunicationThen] = //: return Array<PhaticCommunicationThen>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [PhaticCommunicationThen] = //: return Array<PhaticCommunicationThen>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [PhaticCommunicationThen] = //: return Array<PhaticCommunicationThen>()
        .init()
    //: }()
}
...
extension DelayListManager {
    /// 初始化数据
    //: func req_moreMsgInitData() {
    func digitiserLevelModelDataStatus() {
        //: self.moreMsgArr = DelayConversationListener.shared.moreMsgList
        self.moreMsgArr = DelayConversationListener.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func ghb(indexPath: IndexPath) -> PhaticCommunicationThen? {
    ...
    func modelFile() -> PhaticCommunicationThen {
        // 更新更多消息最新数据
        //: req_moreMsgInitData()
        digitiserLevelModelDataStatus()
        ...
        moreMsgModel.unreadCount = barChat()
...
extension DelayListManager {
    ...
    func doingCompletion(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        // 批量请求在线状态
        //: MusicInfoManager.cache_getChatUserStatus(with: data) { resultArr, _ in
        MusicInfoManager.funding(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(kClickData.lowercased())].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(String(k_inputName) + String(k_voiceValue.prefix(7)))].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(k_sColorTitle.prefix(8)) + k_sessionData.replacingOccurrences(of: "bubble", with: "r"))].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(k_centerContent.replacingOccurrences(of: "title", with: "i") + String(kAddData.suffix(7)))].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(String(k_aConnectName) + String(k_edgeText))].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(kTableArcText.replacingOccurrences(of: "cell", with: "r") + "Status")].int ?? 1
                    ...
                    self.needStart(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func listEqual(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(kClickData.lowercased())] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(String(k_inputName) + String(k_voiceValue.prefix(7)))] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(k_sColorTitle.prefix(8)) + k_sessionData.replacingOccurrences(of: "bubble", with: "r"))] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(kTableArcText.replacingOccurrences(of: "cell", with: "r") + "Status")] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((kTableArcText.replacingOccurrences(of: "cell", with: "r") + "Status")) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension DelayListManager {
    ...
    func noteWith() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in DelayConversationListener.shared.norConvList {
        for model in DelayConversationListener.shared.norConvList {
            //: if model.totalIntimate >= RowReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= RowReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.alter(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in DelayConversationListener.shared.topConvList {
        for model in DelayConversationListener.shared.topConvList {
            //: if model.totalIntimate >= RowReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= RowReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func gossipmongering(convModel: PhaticCommunicationThen, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func imbecility(indexPath: IndexPath) -> PhaticCommunicationThen? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func needStart(uid: String, intimate: Int) {
        //: guard intimate > 0 else { return }
        guard intimate > 0 else { return }
        //: MusicInfoManager.cache_updateIntimate(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
        MusicInfoManager.keyList(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func viewEvent(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(kClickData.lowercased())] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(k_priceName.replacingOccurrences(of: "me", with: "in"))] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
