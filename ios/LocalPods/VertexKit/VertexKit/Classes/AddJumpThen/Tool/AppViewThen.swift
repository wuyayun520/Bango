...
/*: "gifts" :*/
fileprivate let k_cookieTitle:String = "GIFTS"

/*: "vipGifts" :*/
fileprivate let k_viewText:[Character] = ["v","i","p","G","i"]
fileprivate let k_userText:[Character] = ["f","t","s"]

/*: "packGifts" :*/
fileprivate let kTableValue:String = "count return self videopackGi"
fileprivate let kColorText:String = "names"

/*: "giftSendItem" :*/
fileprivate let k_modelValue:String = "gviewt"
fileprivate let kInsideTitle:String = "Itembottom let finish"
...
class AppViewThen: NSObject {
    ...
    private var cacheDict = [String: [SelectedInsetTarget: [Any]]]()
    // 数据锁
    ...
    private var giftType = ColorScalarLiteral.normal

    //: static let share = TalkingChatGiftManager()
    static let share = AppViewThen()
...
extension AppViewThen {
    ...
    func quality(type: ColorScalarLiteral = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        AppViewThen.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[SelectedInsetTarget.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            AppViewThen.share.female(completion: { isSucceed in
            ...
            AppViewThen.share.followUser(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            AppViewThen.share.ceremony(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            AppViewThen.share.phoneError(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func phoneError(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = AppViewThen.share.giftType
        //: SourceRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        SourceRequestTool.nearTouchSend(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [SelectedInsetTarget: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(k_cookieTitle.lowercased())] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<BagHandyJSON>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[SelectedInsetTarget.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(k_viewText) + String(k_userText))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<BagHandyJSON>.deserialize(from: array as? Array) {
                    ...
                    dataDict[SelectedInsetTarget.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(kTableValue.suffix(6)) + kColorText.replacingOccurrences(of: "name", with: "ft"))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<BagHandyJSON>.deserialize(from: array as? Array) {
                    ...
                    dataDict[SelectedInsetTarget.Bags] = bagsArr
                }
                ...
                var firstModel = TalkingGiftTitleTheoryModelType()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(k_modelValue.replacingOccurrences(of: "view", with: "if") + "Send" + String(kInsideTitle.prefix(4)))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<TalkingGiftTitleTheoryModelType>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func female(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = AppViewThen.share.giftType
        //: SourceRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        SourceRequestTool.sightCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func sharedTo(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: SourceRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        SourceRequestTool.addStop(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<DetailReactiveCompatible>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func followUser(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = ColorScalarLiteral.live
        //: SourceRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        SourceRequestTool.depend(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func ceremony(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = ColorScalarLiteral.party
        //: SourceRequestTool.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        SourceRequestTool.noneTo(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func quitType(perPageCount: Int, dataType: SelectedInsetTarget) -> Int {
    ...
    func addState(dataType: SelectedInsetTarget) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func tableShow(dataType: SelectedInsetTarget, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension AppViewThen {
    ...
    private func serveMemory() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(pushExcept),
                                               //: name: GIFT_REFRESHPACKAGE_NOTIFICATION,
                                               name: kLet_streamName,
                                               //: object: nil)
                                               object: nil)
    }
...
extension AppViewThen {
    ...
    func mid(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        AppViewThen.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
