...
fileprivate let k_voiceContent:[UInt8] = [0xf8,0xff,0xf8,0xe5,0xb9,0xf2,0xfe,0xf5,0xf4,0xe3,0xab,0xb8,0xb1,0xf9,0xf0,0xe2,0xb1,0xff,0xfe,0xe5,0xb1,0xf3,0xf4,0xf4,0xff,0xb1,0xf8,0xfc,0xe1,0xfd,0xf4,0xfc,0xf4,0xff,0xe5,0xf4,0xf5]

private func imageHiddenMatch(label num: UInt8) -> UInt8 {
    return num ^ 145
}

/*: "%@.mp4" :*/
fileprivate let k_pointData:String = "%@.mp4kit empty time official"

/*: "http" :*/
fileprivate let kBeanTitle:String = "httpolicy"
...
class AppearPlayDelegate: TalkingEventItemView {
	var searchName: String = "video"
	var tableOfTitlesArray: [AnyHashable] = []
	var recordDictionary: [AnyHashable: String] = [:]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_voiceContent.map{imageHiddenMatch(label: $0)}, encoding: .utf8)!)
    }
    ...
    public lazy var vapPlayerView: UIView? = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(sendCurrent))
    ...
    override func removeAnimating() {
        //: super.startAnimating()
        super.removeAnimating()
        ...
        let path = SymbolReactiveCompatible.shared.stochasticProcessAbove(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? StochasticProcessNameTalkingMeasurable())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (String(k_pointData.prefix(6))), path)
            ...
            self.delegate?.fillDoingTitle(effectItemView: self)
        }
    ...
    @objc func sendCurrent() {}
}

///  --- HWDMP4PlayDelegate

//: extension TalkingVapAnimationView: HWDMP4PlayDelegate {
extension AppearPlayDelegate: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context _: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((kBeanTitle.replacingOccurrences(of: "policy", with: "p"))) {
