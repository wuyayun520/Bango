...
/*: "id" :*/
fileprivate let kUserContent:String = "ID"

/*: "num" :*/
fileprivate let kDataValue:[Character] = ["n","u","m"]
...
@objc public enum PopScalar: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct TalkingBeginHandyJSON: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func fillSetNum(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct NameBagAnimatModel: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(kDataValue))
    }
}
