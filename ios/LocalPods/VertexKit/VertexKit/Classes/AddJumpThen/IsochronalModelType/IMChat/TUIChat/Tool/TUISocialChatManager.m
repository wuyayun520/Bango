...
+ (TUIMessageCellData *)insertTXMessageModelWithExtral:(NSDictionary *)extral
    ...
    if (data != nil) {
        [[DelayConversationListener shared] likeContentWithTargetId:toUid msgCellData:data isPrivete:YES];
    }
    return data;
}
...
+ (TUIMessageCellData *)handleTXMessageSendModelWithExtral:(NSDictionary *)extral
    ...
    if (isTip) {
        ...
        PanAwakeModel *model = [[PanAwakeModel alloc]init];
        LengthModel *tipModel = [LengthModel modelWithDic:extralDict];
        model.tips = tipModel;
        data.msgModel = model;
        return data;
    }
    ...
    tempDict[@"user"] = @{
        @"uid":RowReactiveCompatible.share.loginUserMode.userID,
        @"nickname":RowReactiveCompatible.share.loginUserMode.nickname,
        @"headPic":RowReactiveCompatible.share.loginUserMode.headPic,
        @"age":RowReactiveCompatible.share.loginUserMode.age
    };
    ...
    if ([msgInfo[@"msgType"] isEqualToString:@"gift"]) {
        ...
        if ([msgInfo[@"isGroup"] isEqualToString:@"1"]) { // 群聊
            data = [[MusicCellData alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        } else {
            data = [[SourceCellData alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        }
        
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        PanAwakeModel *model = [PanAwakeModel modelWithDic:extral];
        data.msgModel = model;
    ...
    if ([msgInfo[@"msgType"] isEqualToString:@"WantGift"]) {
        TUIMessageCellData *data = nil;
        data = [[TitleReactiveCompatible alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        PanAwakeModel *model = [PanAwakeModel modelWithDic:extral];
        data.msgModel = model;
    ...
    if ([msgInfo[@"msgType"] isEqualToString:@"txt"]) {
        TUIMessageCellData *data = nil;
        if ([msgInfo[@"isGroup"] isEqualToString:@"1"]) { // 群聊
            data = [[AddCellData alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        } else {
            data = [[BeginCellData alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        }
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        PanAwakeModel *model = [PanAwakeModel modelWithDic:extral];
    ...
    if ([msgInfo[@"msgType"] isEqualToString:@"img"]) {
        ClickCellData *data = [[ClickCellData alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        PanAwakeModel *model = [PanAwakeModel modelWithDic:extral];
        data.msgModel = model;
    ...
    if ([msgInfo[@"msgType"] isEqualToString:@"video"]) {
        ColorThen *data = [[ColorThen alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        PanAwakeModel *model = [PanAwakeModel modelWithDic:extral];
        data.msgModel = model;
    ...
    if ([msgInfo[@"msgType"] isEqualToString:@"audio"]) {
        LengthReactiveCompatible *data = [[LengthReactiveCompatible alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        PanAwakeModel *model = [PanAwakeModel modelWithDic:extral];
        data.msgModel = model;
    ...
    if (extral[@"gift"] != nil && [msgInfo[@"messageType"] integerValue] == 4) {
        BeginCellData *data = [[BeginCellData alloc] initWithDirection:isSend? MsgDirectionOutgoing:MsgDirectionIncoming];
        NSData *cData = [extraString dataUsingEncoding:NSUTF8StringEncoding];
        data.innerMessage = [[V2TIMManager sharedInstance] createCustomMessage:cData];
        PanAwakeModel *model = [PanAwakeModel modelWithDic:extral];
        data.msgModel = model;
...
+ (TUIMessageCellData *)handleTXMessageReSendModelWithData:(TUIMessageCellData *)cellData {
    // 图片
    if ([cellData isKindOfClass:[ClickCellData class]]) {
        ClickCellData *reCellData = (ClickCellData *)cellData;
        NSArray *path = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
        NSString *filePath = [path objectAtIndex:0];
        filePath = [filePath stringByAppendingPathComponent:[NameMacroDefine imageWith]];
    ...
    // 视频
    if ([cellData isKindOfClass:[ColorThen class]]) {
        ColorThen *reCellData = (ColorThen *)cellData;
    ...
    // 语音
    if ([cellData isKindOfClass:[LengthReactiveCompatible class]]) {
        LengthReactiveCompatible *reCellData = (LengthReactiveCompatible *)cellData;
        SmallnessMsgTable *voiceModel = [MusicInfoManager cacheInCommunicationWithMessage:reCellData.innerMessage];
        NSMutableDictionary * dic = [NSMutableDictionary dictionary];
        ...
        [msgInfo setObject:RowReactiveCompatible.share.loginUserMode.userID forKey:@"senderId"];
        [dic setObject:msgInfo forKey:@"msgInfo"];
    ...
    // 文本
    if ([cellData isKindOfClass:[BeginCellData class]]) {
        BeginCellData *reCellData = (BeginCellData *)cellData;
        NSMutableDictionary *dic = [NSMutableDictionary dictionary];
        NSMutableDictionary *params = [NSMutableDictionary dictionary];
        {
            NSMutableDictionary * msgInfo = [NSMutableDictionary dictionary];
            ...
            if (reCellData.msgModel.quoteMsgInfo.localSize) { // 引用消息重发
...
+ (TUIMessageCellData *)handleTXReceiveMessageModelWithExtral:(NSDictionary*)dic
    ...
    if ([user[@"id"] integerValue] != RowReactiveCompatible.share.loginUserMode.userID.integerValue) {
        uid = user[@"id"];
    }
    
    PanAwakeModel *model = [PanAwakeModel modelWithDic:dic[@"extra"]];
    BOOL isSelf = (model.user.uid == RowReactiveCompatible.share.loginUserMode.userID.integerValue);
    
    //后台代发送的自动打招呼消息
    BOOL isAutoSendMsg = NO;
    if (model.msgInfo.uid.integerValue == RowReactiveCompatible.share.loginUserMode.userID.integerValue && model.msgInfo.messageType == 3) {
        isSelf = YES;
        isAutoSendMsg = YES;
    }
    ...
    if ([dic[@"extra"][@"msgType"] isEqualToString:@"system"] || contenType == TXMessageContentType_tips) {
        PanAwakeModel *model = [[PanAwakeModel alloc]init];
        LengthModel *tipModel = [LengthModel modelWithDic:dic[@"extra"]];
    ...
    if (contenType == TXMessageContentType_picture) {
        cellData = [[ClickCellData alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        
    } else if (contenType == TXMessageContentType_video) {
        cellData = [[ColorThen alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        
    } else if(contenType == TXMessageContentType_gift) {
        if (tempModel.groupID != nil && tempModel.groupID.length > 0) { // 群聊gift
            cellData = [[MusicCellData alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        } else { // 私聊gift
            cellData = [[SourceCellData alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        }
        ...
        cellData = [[TitleReactiveCompatible alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        if (msgInfo.allKeys.count > 0) {
            model.msgInfo = [ChatModel modelWithDic:msgInfo];
        }
        cellData.msgModel = model;
        
    } else if (contenType == TXMessageContentType_voice) {
        cellData = [[LengthReactiveCompatible alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        LengthReactiveCompatible *audioCellData = (LengthReactiveCompatible *)cellData;
        if (audioCellData.voiceModel.db_voiceCacheWrap.msgId.length == 0) {
            ...
            SmallnessMsgTable *cacheWrap = [SmallnessMsgTable fileWithWith:mainKey];
            if (!cacheWrap) {
                SmallnessMsgTable *wrap = [SmallnessMsgTable nameDb:[TUISocialChatManager packageDictWithTXMessage:tempModel]];
        ...
        cellData = [[IndexCellData alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        
    } else {
        if ([dic[@"extra"][@"title"] length] > 0 ||
            [dic[@"extra"][@"content"] length] > 0 ||
            [dic[@"extra"][@"img"] length] > 0) { // 系统新消息
            cellData = [[AddRowCellData alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
            ...
            cellData = [[BeginCellData alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
        }
    }
    ...
    if (model.user == nil) {
        ...
        HighProfileModel *user = [HighProfileModel modelWithDic:userDict];
        ...
        if (isAutoSendMsg) {
            ...
            user.headPic = RowReactiveCompatible.share.loginUserMode.headPic;
        }
        model.user = user;
    }
    if (model.msgInfo == nil) {
        NSMutableDictionary *extraDict = [NSMutableDictionary dictionaryWithDictionary:dic[@"extra"]];
        ChatModel *msgInfo = [[ChatModel alloc]init];
    ...
    if(contenType == TXMessageContentType_gift && tempModel.groupID != nil && tempModel.groupID.length > 0) {
        if (model.toUser == nil) { // 送礼所有人
            PartJsonModel *toUserInfo = [[PartJsonModel alloc]init];
            toUserInfo.uid = @"all";
    ...
    // 私密照片Cache
    if (contenType == TXMessageContentType_picture) {
        NSDictionary *dict = [TalkingAreaOutlookJsonModel belowName];
        if ([dict objectForKey:tempModel.msgID]) {
            cellData.msgModel.msgInfo.intimatePhoto = [[TalkingAreaOutlookJsonModel alloc] initWithString:dict[tempModel.msgID] error:nil];
        }
    }
    
    // 私密视频Cache
    if (contenType == TXMessageContentType_video) {
        NSDictionary *dict = [LowlinessModel epicycleDict];
        if ([dict objectForKey:tempModel.msgID]) {
            cellData.msgModel.msgInfo.video = [[LowlinessModel alloc] initWithString:dict[tempModel.msgID] error:nil];
        }
    }
    
    // 索要礼物Cache
    if (contenType == TXMessageContentType_askforGift) {
        NSArray *arr = [SharedJsonModel session];
...
+ (TXMessageContentType )messageCellType:(NSDictionary *)extraDic {
    ...
    PanAwakeModel *model = [PanAwakeModel modelWithDic:dic];
...
+ (NSDictionary *)packageDictWithTXMessage:(V2TIMMessage *)message {
    ...
    //自己发送的和接收的数据结构不一样
    if ([senderId isEqualToString:RowReactiveCompatible.share.loginUserMode.userID]) {
    ...
    info[@"isRead"] = [NSString stringWithFormat:@"%d",[senderId isEqualToString:RowReactiveCompatible.share.loginUserMode.userID]];
    return [info copy];
}
