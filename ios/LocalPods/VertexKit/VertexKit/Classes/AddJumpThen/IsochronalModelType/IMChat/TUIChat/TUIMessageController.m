...
- (void)setupViews {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(menuWillHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onNewMessage:) name:[NotificationRakeThen toWindowNoti] object:nil];
...
- (void)loadMessage {
    ...
    if (self.currentChatType == TalkingIMChatType_group ||
        ...
        [[V2TIMManager sharedInstance] getGroupHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([DelayConversationListener shared].isConnection) { //推送点进来会报错
                NSString *str = [@"Net Error, Try again later" windowThen];
                [self appearShowWithShowMsg:[str windowThen]];
            }
        ...
        [[V2TIMManager sharedInstance] getC2CHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            if ([DelayConversationListener shared].isConnection) {                
                if (RowReactiveCompatible.share.appStatus == 0) {
                    NSString *str = [@"Net Error, Try again later" windowThen];
                    [self appearShowWithShowMsg:[str windowThen]];
                }
            }
...
- (void)getMessages:(NSArray *)msgs msgCount:(int)msgCount {
    ...
    for (NSInteger k = array.count - 1; k >= 0; --k) {
        ...
        if ([userId isEqualToString:RowReactiveCompatible.share.loginUserMode.userID]) {
...
- (void)onNewMessage:(NSNotification *)notification
{
    ...
    if ([msg.sender isEqualToString:RowReactiveCompatible.share.loginUserMode.userID]) {
        //过滤本人发送的消息
        return;
    }
    ...
    if (uiMsgs.count > 0) {
        ...
        if ((self.tableView.contentSize.height - self.tableView.contentOffset.y) < [NameMacroDefine lowness] * 1.5) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //    缓存存在问题待处理
    //    if (height > 0 && ![data isKindOfClass:[AbIndexCellData class]] && ![data isKindOfClass:[UserCellData class]]) {
    //        return height;
    //    }
...
- (void)sendCustomMessage:(TUIMessageCellData *)msg {
    ...
    NSInteger sender = RowReactiveCompatible.share.loginUserMode.userID.integerValue;
    NSString *msgID;
    if (isGroup) {
...
- (void)modifyCustomMessageDataWithExtral:(NSDictionary *)extral cellData:(TUIMessageCellData *)msg {
    ...
    // 更新内存模型
    msg.msgModel = [PanAwakeModel modelWithDic:newExtraDict];
}
...
///   - msg: 图片消息
///   - model: 私密照片模型
- (void)changeIntimatePhotoMsg:(TUIMessageCellData *)msg photoModel:(TalkingAreaOutlookJsonModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [TalkingAreaOutlookJsonModel table:model theLikesOfMain:msg.msgID];
...
///   - msg: 视频消息
///   - model: 私密视频模型
- (void)changeIntimateVideoMsg:(TUIMessageCellData *)msg videoModel:(LowlinessModel *)model {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (model == nil) { return; }
        [LowlinessModel errorKey:model status:msg.msgID];
...
- (void)changeIntimateGiftMsg:(TUIMessageCellData *)msg {
    dispatch_async(dispatch_get_main_queue(), ^{
        [SharedJsonModel sumimate:msg.msgID];
...
- (TalkingSystemMsgCellData *)transSystemMsgFromDate:(NSDate *)date
{
    if(_msgForDate == nil || fabs([date timeIntervalSinceDate:_msgForDate.timestamp]) > MAX_MESSAGE_SEP_DLAY) {
        ...
        system.contentStr = [NSDate colouringWithShowDetail:YES date:date];
...
- (void)onLongPressMessageWithCell:(TUIMessageCell *)cell{
    ...
    BOOL hasJumps = NO;
    if ([data isKindOfClass:[BeginCellData class]]) {
        BeginCellData *textCellData = (BeginCellData *)data;
    ...
    // 只有文字才有复制功能
    if (([data isKindOfClass:[AddCellData class]] ||
         [data isKindOfClass:[BeginCellData class]]) && hasJumps == NO) {
        // 过滤掉音视频通话文本消息
        if (data.msgModel.msgInfo.callCmd == nil) {
            [items addObject:[[UIMenuItem alloc] initWithTitle:@"Copy".windowThen action:@selector(onCopyMsg:)]];
            // 私聊 && 接收消息 && 消息未翻译成功
            if ([data isKindOfClass:[BeginCellData class]] &&
                data.direction == MsgDirectionIncoming &&
                data.translateStatus != MsgTranslate_succeed) {
                [items addObject:[[UIMenuItem alloc] initWithTitle:@"Trans".windowThen action:@selector(onTranslateMsg:)]];
            }
        }
    }
    
    if ([self canReplyResponseWithLongGestureRecognizer:data]) {
        [items addObject:[[UIMenuItem alloc] initWithTitle:@"Reply".windowThen action:@selector(onReplyMsg:)]];
    }
...
- (NSArray *)requiredResponseWithLongGestureRecognizer {
    return @[[AddCellData class],
             [BeginCellData class],
             [LengthReactiveCompatible class],
             [ClickCellData class],
             [ColorThen class],
             [SourceCellData class],
             [TitleReactiveCompatible class]];
}
...
- (void)onCopyMsg:(id)sender {
    ...
    if ([_menuUIMsg isKindOfClass:[AddCellData class]] ||
        [_menuUIMsg isKindOfClass:[BeginCellData class]]) {
        BeginCellData *txtMsg = (BeginCellData *)_menuUIMsg;
...
- (BOOL)canReplyResponseWithLongGestureRecognizer:(TUIMessageCellData *)cellData {
    ...
    if ([cellData isKindOfClass:[SourceCellData class]] || [cellData isKindOfClass:[LengthReactiveCompatible class]]) {
        return YES;
    }
...
- (void)translateMsgWithCellData:(TUIMessageCellData *)cellData {
    ...
    if (RowReactiveCompatible.share.appUserConfigMode.enableTranslate == NO) { // 非会员，无法使用翻译
        [[MiraclePushManager share] dataConverterVc];
        return;
    }
    
    if ([cellData isKindOfClass:[BeginCellData class]]) {
        BeginCellData *txtMsg = (BeginCellData *)cellData;
        [self translateMessageToReq:txtMsg andType:0];
        
    } else if ([cellData isKindOfClass:[AddCellData class]]) {
        AddCellData *txtMsg = (AddCellData *)cellData;
...
- (void)translateMessageToReq:(TUIMessageCellData *)txtMsg andType:(int)type {
    ...
    // 调用翻译接口
    [SourceRequestTool dataAddWithMsgId:txtMsg.msgModel.msgInfo.msgId type:type completion:^(BOOL succeed, NSDictionary *dict, NSInteger errorCode) {
        if (!succeed) {
            if (errorCode == 403) { // 非会员，无法使用翻译
                RowReactiveCompatible.share.appUserConfigMode.enableTranslate = NO;
                [[MiraclePushManager share] dataConverterVc];
            }
