...
fileprivate let kBroadcastViewForValue:[UInt8] = [0xd7,0xd0,0xd7,0xca,0x96,0xdd,0xd1,0xda,0xdb,0xcc,0x84,0x97,0x9e,0xd6,0xdf,0xcd,0x9e,0xd0,0xd1,0xca,0x9e,0xdc,0xdb,0xdb,0xd0,0x9e,0xd7,0xd3,0xce,0xd2,0xdb,0xd3,0xdb,0xd0,0xca,0xdb,0xda]

private func imageSideTop(model num: UInt8) -> UInt8 {
    return num ^ 190
}

/*: "icon_chat_miss" :*/
fileprivate let kEqualData:[Character] = ["i","c","o","n","_","c","h","a","t","_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let k_indexData:String = "icon_cif image to white layer"
...
// __CLOSE_PRINT__
//
//  DerivationReactiveCompatible.swift
//  AbroadTalking
//
...
class DerivationReactiveCompatible: VideoMsgCell {
	var signalQuantity: Int = 98
	var manualCount: Double = -42.6
	var scaleTitle: String = "text"
	var haphazardDictionary: [AnyHashable: String] = [:]
	var offQuantity: Int = 79
	var videoQuantity: Double = 74.2
	var noteTitle: String = "main"
	var transitionWillDictionary: [AnyHashable: String] = [:]

//    var transBlock: (() -> Void)?

    //: var textData: BeginCellData?
    var textData: BeginCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kBroadcastViewForValue.map{imageSideTop(model: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var quoteView: ComparableThen = {
        //: let quoteView = TalkingMsgQuoteView()
        let quoteView = ComparableThen()
        //: quoteView.isHidden = false
        quoteView.isHidden = false
        //: quoteView.addTarget(self, action: #selector(quoteBtnClick), for: .touchUpInside)
        quoteView.addTarget(self, action: #selector(effectQuoteTarget), for: .touchUpInside)
        //: return quoteView
        return quoteView
        //: }()
    }()
...
extension DerivationReactiveCompatible {
    //: override func fill(with data: TCommonCellData) {
    override func fill(with data: TCommonCellData) {
        //: super.fill(with: data)
        super.fill(with: data)

        //: if let temTextData = data as? BeginCellData {
        if let temTextData = data as? BeginCellData {
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != NameMacroDefine.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.localSize() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.collectionName(name: (String(kEqualData)))
                ...
                self.transStatusImagV.image = UIImage.collectionName(name: (String(k_indexData.prefix(6)) + "hat_yes"))
            }
    ...
    func my() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(finishPath(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == NameMacroDefine.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == NameMacroDefine.have())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            empty()
