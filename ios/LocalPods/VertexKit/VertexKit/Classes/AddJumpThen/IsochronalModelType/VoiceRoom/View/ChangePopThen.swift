...
fileprivate let k_numberContent:[UInt8] = [0x60,0x67,0x60,0x7d,0x21,0x6a,0x66,0x6d,0x6c,0x7b,0x33,0x20,0x29,0x61,0x68,0x7a,0x29,0x67,0x66,0x7d,0x29,0x6b,0x6c,0x6c,0x67,0x29,0x60,0x64,0x79,0x65,0x6c,0x64,0x6c,0x67,0x7d,0x6c,0x6d]

/*: "Online Users" :*/
fileprivate let k_modeTitle:String = "Online else after if"
fileprivate let kOfValue:String = "Usersinput document if color in"

/*: "CircumferenceViewCell" :*/
fileprivate let k_atButtonName:[Character] = ["T","a","l","k","i","n","g","L","i","v","e"]
fileprivate let kRestoreContent:[Character] = ["R","o","o","m","O","n","l","i","n","e","L","i","s"]
fileprivate let kColorValue:[Character] = ["t","C","e","l","l"]
...
class ChangePopThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_numberContent.map{$0^9}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(metronymicClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(k_modeTitle.prefix(7)) + String(kOfValue.prefix(5))).localized
        //: return lb
        return lb
        //: }()
    }()
...
extension ChangePopThen {
    ...
    private func vertical() {
        ...
        AddReqTool.standing(roomId: EventManager.bracketOut().partyModel.roomId) { succeed, result, _ in
            ...
            if let datas = [ViewTextTheoryListModel].deserialize(from: array as? Array) {
...
extension ChangePopThen {
    ...
    @objc private func metronymicClick() {
        //: dismiss()
        infoDismiss()
    }
    ...
    func bring() {
        ...
        contentView.frame = CGRect(x: 0, y: kLet_nameValue, width: kLet_errData, height: contentV_H)
        //: reqData()
        vertical()
...
extension ChangePopThen: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = (String(k_atButtonName) + String(kRestoreContent) + String(kColorValue))
        ...
        cell?.computerSimulation(model: DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
