...
fileprivate let k_dataViewName:[UInt8] = [0xb0,0xb7,0xb0,0xad,0xf1,0xba,0xb6,0xbd,0xbc,0xab,0xe3,0xf0,0xf9,0xb1,0xb8,0xaa,0xf9,0xb7,0xb6,0xad,0xf9,0xbb,0xbc,0xbc,0xb7,0xf9,0xb0,0xb4,0xa9,0xb5,0xbc,0xb4,0xbc,0xb7,0xad,0xbc,0xbd]

/*: "Select mute duration" :*/
fileprivate let kSpaceValue:[Character] = ["S","e","l","e","c","t"," ","m","u","t"]
fileprivate let k_selectTitle:String = "e view actual var view"
fileprivate let k_messageValue:[Character] = ["d","u","r","a","t","i","o","n"]

/*: "Cirform" :*/
fileprivate let k_toSearchedText:String = "Cirformself line as"

/*: "Mute success" :*/
fileprivate let kLayerName:[Character] = ["M","u","t","e"," ","s","u","c","c","e","s","s"]

/*: "day" :*/
fileprivate let k_viewTopName:String = "dstop"

/*: "days" :*/
fileprivate let kFrameName:String = "dcosys"

/*:  " :*/
fileprivate let k_stylePlayerValue:String = " "
...
class ElectronicTextView: UIView {
    ...
    var viewType: WithViewType = .live
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_dataViewName.map{$0^217}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(stickTo), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        //: let lb = UILabel()
        let lb = UILabel()
        //: lb.font = UIFont.moleculeClick(fontSize: 18)
        lb.font = UIFont.moleculeClick(fontSize: 18)
        ...
        lb.text = (String(kSpaceValue) + String(k_selectTitle.prefix(2)) + String(k_messageValue)).localized
        //: lb.numberOfLines = 0
        lb.numberOfLines = 0
        //: return lb
        return lb
    ...
    lazy var commitBtn: UIButton = {
        ...
        btn.setTitle((String(k_toSearchedText.prefix(7))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(givenName), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ElectronicTextView {
    /// 消除视图按钮
    //: @objc private func bgDismissButtonClick() {
    @objc private func stickTo() {
        //: dismiss()
        requestDismiss()
    }
    ...
    func dateView() {
        ...
        self.contentView.frame = CGRect(x: (kLet_errData - contentV_Size.width) / 2, y: kLet_nameValue, width: contentV_Size.width, height: contentV_Size.height)

        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y = (ScreenHeight-self.contentV_Size.height)/2
            self.contentView.frame.origin.y = (kLet_nameValue - self.contentV_Size.height) / 2
        }
    }
    ...
    func requestDismiss() {
        ...
        UIView.animate(withDuration: 0.3) {
            ...
            self.contentView.frame.origin.y = kLet_nameValue
...
extension ElectronicTextView {
    ...
    @objc private func scale(btn: UIButton) {
    ...
    @objc private func givenName() {
        ...
        if self.viewType == .live {
            ...
            VehicleAudienceManager().gape(targetUid: targetUid, duration: index) { succeed, _, _ in
                //: ProgressHUD.dismiss()
                ViewBeginReactiveCompatible.capability()
                //: if succeed {
                if succeed {
                    //: self.func__showStatusBarSuccessMsg(showMsg: "Mute success".localized)
                    self.viewLog(showMsg: (String(kLayerName)).localized)
                }
                //: self.dismiss()
                self.requestDismiss()
            }
            ...
            AddReqTool.windowVoiceCompletion(targetUid: targetUid, roomID: EventManager.bracketOut().partyModel.roomId, duration: index) { succeed, _, _ in
                //: ProgressHUD.dismiss()
                ViewBeginReactiveCompatible.capability()
                //: if succeed {
                if succeed {
...
extension ElectronicTextView {
    ...
    private func disappearOn() {
        ...
        for i in 0 ..< 3 {
            ...
            btn.australianState(color: UIColor.cellColor(), forState: .selected)
            //: btn.titleLabel?.font = UIFont.moleculeClick(fontSize: 16)
            btn.titleLabel?.font = UIFont.moleculeClick(fontSize: 16)
            //: let title = titleStr[i]
            let title = titleStr[i]
            //: let dayStr = (title == "1") ? "day".localized:"days".localized
            let dayStr = (title == "1") ? (k_viewTopName.replacingOccurrences(of: "stop", with: "ay")).localized : (kFrameName.replacingOccurrences(of: "cos", with: "a")).localized
            //: btn.setTitle("\(title) " + dayStr, for: .normal)
            btn.setTitle("\(title) " + dayStr, for: .normal)
            //: btn.tag = 100+i
            btn.tag = 100 + i
            //: btn.addTarget(self, action: #selector(muteBtnClick), for: .touchUpInside)
            btn.addTarget(self, action: #selector(scale), for: .touchUpInside)
