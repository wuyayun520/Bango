...
/*: s" :*/
fileprivate let k_viewData:String = "corner"

/*: "init(coder:) has not been implemented" :*/
fileprivate let k_emptyContent:[UInt8] = [0x37,0x30,0x37,0x2a,0x76,0x3d,0x31,0x3a,0x3b,0x2c,0x64,0x77,0x7e,0x36,0x3f,0x2d,0x7e,0x30,0x31,0x2a,0x7e,0x3c,0x3b,0x3b,0x30,0x7e,0x37,0x33,0x2e,0x32,0x3b,0x33,0x3b,0x30,0x2a,0x3b,0x3a]

/*: "bth_live_follow_pre" :*/
fileprivate let kModelName:String = "bth_lcollection var view"
fileprivate let kPathData:String = "ive_follgift name"
fileprivate let kViewContent:String = "return to layer self liveow_pre"

/*: "btn_live_mini_nor" :*/
fileprivate let kMainValue:String = "name extension remove image databtn_li"
fileprivate let kFlushTimeSumValue:String = "oold"

/*: "#2DF2FF" :*/
fileprivate let kEqualFrameText:String = "empty return for mode#2DF2FF"

/*: "Live" :*/
fileprivate let kActionValue:[Character] = ["L","i","v","e"]

/*: "#FF506D" :*/
fileprivate let k_appData:String = "type sound button result content#FF50"
fileprivate let kMarginTitle:String = "6Dself shared handle"

/*: "  :*/
fileprivate let k_makeTitle:[Character] = [" "]

/*: "removeAttentionUid" :*/
fileprivate let k_eventText:String = "errormov"
fileprivate let k_cellData:String = "if color falseionUid"

/*: "attentionUid" :*/
fileprivate let k_selectedSystemText:String = "atexte"
fileprivate let k_userValue:[Character] = ["i","d"]

/*: "source" :*/
fileprivate let k_makeSizeValue:[Character] = ["s","o","u","r","c","e"]

/*: "num" :*/
fileprivate let kFirstText:String = "nfillm"

/*: "UID: :*/
fileprivate let k_fireLogName:String = "UID:label var app live let"

/*: "btn_live_closed_nor" :*/
fileprivate let k_tagContactData:String = "random status height progress requestbtn_"
fileprivate let k_theText:[Character] = ["c","l","o","s","e","d","_","n","o","r"]
...
class CapBagTopView: UIView {
    //: weak var weakVC: TalkingBaseViewController?
    weak var weakVC: RecordReactiveCompatible?
    // 右侧按钮点击Block
    ...
    var noFaceTime: Int = 0 {
        //: didSet {
        didSet {
            //: if noFaceTime >= RowReactiveCompatible.share.appConfigMode.countdown1 {
            if noFaceTime >= RowReactiveCompatible.share.appConfigMode.countdown1 { // 展示倒计时
                //: self.liveStatusImgV.isHidden = true
                self.liveStatusImgV.isHidden = true
                //: self.countdownView.isHidden = false
                self.countdownView.isHidden = false
                //: self.countdownLab.text = "\(MAX_NOFACETIME-noFaceTime)s"
                self.countdownLab.text = "\(kLet_senseData - noFaceTime)s"
                // svga
                //: self.svgaView.isHidden = false
                self.svgaView.isHidden = false
                //: self.svgaView.refreshView(countdown: "\(MAX_NOFACETIME-noFaceTime)")
                self.svgaView.actionRestore(countdown: "\(kLet_senseData - noFaceTime)")
                //: } else {
            } else {
    ...
    private var liveModel = WorldHandyJSON()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_emptyContent.map{$0^94}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(requestAttribute), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.collectionName(name: (String(kModelName.prefix(5)) + String(kPathData.prefix(8)) + String(kViewContent.suffix(6)))), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(underRequest), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.collectionName(name: (String(kMainValue.suffix(6)) + "ve_mini_n" + kFlushTimeSumValue.replacingOccurrences(of: "old", with: "r"))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(frontEndIndex), for: .touchUpInside)
    ...
    private lazy var liveStatusImgV: AddReactiveCompatible = {
        ...
        let image = UIImage.thoughtImageShowSize(color: UIColor(hex: (String(kEqualFrameText.suffix(7))))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.circle(), for: .normal)
        img.setImage(image.circleContext(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(kActionValue)).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    private lazy var countdownView: UIView = {
        ...
        icon.backgroundColor = UIColor(hex: (String(k_appData.suffix(5)) + String(kMarginTitle.prefix(2))))
    ...
    private lazy var countdownLab: UILabel = {
        ...
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.text = " \(RowReactiveCompatible.share.appConfigMode.countdown2)s"
        lab.text = " \(RowReactiveCompatible.share.appConfigMode.countdown2)s"
    ...
    lazy var svgaView: DecelerateReactiveCompatible = {
        //: let v = TalkingLiveCountdownView()
        let v = DecelerateReactiveCompatible()
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(thenEnableCreate), for: .touchUpInside)
    ...
    private lazy var onlineListView: ColorJumpDataSource = {
        //: let v = TalkingLiveRoomOnlineListView()
        let v = ColorJumpDataSource()
        //: v.touchIconBtnBlock = { [weak self] uid in
        v.touchIconBtnBlock = { [weak self] uid in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: self.showUserCard(uid: uid)
            self.usufructuaryUid(uid: uid)
        }
        //: return v
        return v
        //: }()
...
extension CapBagTopView {
    ...
    @objc func beforeSend(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(k_eventText.replacingOccurrences(of: "error", with: "re") + "eAttent" + String(k_cellData.suffix(6)))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(k_selectedSystemText.replacingOccurrences(of: "text", with: "tt") + "ntionU" + String(k_userValue))] = uid
            //: dict["source"] = "1"
            dict[(String(k_makeSizeValue))] = "1"
        }
        //: SourceRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
...
extension CapBagTopView {
    ...
    @objc private func thenEnableCreate() {
        //: self.onlineListView.showView(chatGroupId: liveModel.chatGroupId)
        self.onlineListView.startAwake(chatGroupId: liveModel.chatGroupId)
    }

    /// 获取
    //: @objc private func getOnlineNumData() {
    @objc private func connectData() {
        //: TalkingAudienceManager().req_liveRoomOnlineNum(chatGroupId: liveModel.chatGroupId, completion:{ succeed, result, errorModel in
        VehicleAudienceManager().miscount(chatGroupId: liveModel.chatGroupId, completion: { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: let num = json["num"].string
            let num = json[(kFirstText.replacingOccurrences(of: "fill", with: "u"))].string
    ...
    private func modelTimer() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(connectData), userInfo: nil, repeats: true)
    }
...
extension CapBagTopView {
    ...
    func pastAddSave(_ liveModel: WorldHandyJSON) {
        ...
        uidLab.text = (String(k_fireLogName.prefix(4))) + "\(liveModel.streamerInfo.uid)"
        ...
        icon.stopDown(urlStr: liveModel.streamerInfo.headPic, placeImg: UIImage.noApp(sex: liveModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        ...
        if isAnchor {
            //: refreshLiveStatus()
            managerSize()
            //: miniBtn.setImage(UIImage.collectionName(name: "btn_live_mini_nor"), for: .normal)
            miniBtn.setImage(UIImage.collectionName(name: (String(kMainValue.suffix(6)) + "ve_mini_n" + kFlushTimeSumValue.replacingOccurrences(of: "old", with: "r"))), for: .normal)
            //: refreshFollow(isAttention: true)
            adAttention(isAttention: true)
            //: } else {
        } else {
            //: miniBtn.setImage(UIImage.collectionName(name: "btn_live_closed_nor"), for: .normal)
            miniBtn.setImage(UIImage.collectionName(name: (String(k_tagContactData.suffix(4)) + "live_" + String(k_theText))), for: .normal)
            //: refreshFollow(isAttention: self.liveModel.streamerInfo.isAttention)
            adAttention(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }

    /// 最小化/关闭直播 按钮点击事件
    //: @objc func miniButtonClick() {
    @objc func frontEndIndex() {
        //: rightBtnBlock?()
        rightBtnBlock?()
        // 是否是主播
        //: let isAnchor = (String(liveModel.streamerInfo.uid) == RowReactiveCompatible.share.loginUid)
        ...
        if isAnchor { // 改为小窗口
            ...
            WithNeighbourReactiveCompatible.clickEqual().withUp(smallWindow: true)
            //: currentViewController()?.navigationController?.popViewController(animated: false)
            belowResume()?.navigationController?.popViewController(animated: false)
        }
    }

    //: @objc private func iconBgButtonClick() {
    @objc private func requestAttribute() {
        //: showUserCard(uid: String(TalkingLiveManager.shared().liveRoomModel.streamerInfo.uid))
        usufructuaryUid(uid: String(WithNeighbourReactiveCompatible.clickEqual().liveRoomModel.streamerInfo.uid))
    }

    /// 展示个人资料卡
    //: private func showUserCard(uid: String) {
    private func usufructuaryUid(uid: String) {
        //: let userCard = TalkingLiveUserCardView(frame: .zero, uid: uid)
        let userCard = DestabilizationCardView(frame: .zero, uid: uid)
        //: userCard.delegate = weakVC as? any TalkingLiveUserCardViewDelegate
        userCard.delegate = weakVC as? any AppearViewDelegate
        //: userCard.show()
        userCard.sizeShow()
    }
    ...
    private func adAttention(isAttention: Bool) {
    ...
    private func managerSize() {
        //: self.liveStatusImgV.isHidden = !TalkingLiveManager.shared().isLive
        self.liveStatusImgV.isHidden = !WithNeighbourReactiveCompatible.clickEqual().isLive
    ...
    @objc private func underRequest() {
        //: req_followRequest(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
        beforeSend(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.adAttention(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
}
...
extension CapBagTopView {
    ...
    private func setupWithoutConstraintEqual() {
        //: self.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: StatusBarHeight+actualWidth(w: 68))
        self.frame = CGRect(x: 0, y: 0, width: kLet_errData, height: kLet_topData + actualWidth(w: 68))
