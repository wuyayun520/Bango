...
/*: "99+" :*/
fileprivate let k_blockMinData:String = "toptop+"

/*: "init(coder:) has not been implemented" :*/
fileprivate let kClickAwakeTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#2DF2FF" :*/
fileprivate let kFrameValue:[Character] = ["#"]
fileprivate let k_indexPendingValue:String = "2DF2FFfirst let let bottom bean"

/*: "Live" :*/
fileprivate let k_contentData:String = "total name player dismissLive"

/*: "img_home_shadow_icon" :*/
fileprivate let kSectionData:String = "img_hmodel gift"
fileprivate let kCurrentValue:[Character] = ["d","o","w","_","i","c","o","n"]

/*: "get json error" :*/
fileprivate let k_stopData:String = "photo aspect var dataget j"

/*: "#FF2348" :*/
fileprivate let k_hideTitle:String = "path in#FF2348"

/*: "#35C759" :*/
fileprivate let k_infoCenterValue:String = "max location let label#35C7"
fileprivate let k_cellName:[Character] = ["5","9"]

/*: "mfBean" :*/
fileprivate let kFillTitle:[Character] = ["m","f","B","e","a","n"]

/*: "%.1f" :*/
fileprivate let k_sourceData:[Character] = ["%",".","1","f"]

/*: "points\n≈ $" :*/
fileprivate let kButtonTitle:String = "nameoint"
fileprivate let kEmptyName:String = "s\n\u{2248} $"

/*:   :*/
fileprivate let kIncomeText:String = "click"
...
public var kLet_senseData: Int {
    //: return RowReactiveCompatible.share.appConfigMode.countdown1 + RowReactiveCompatible.share.appConfigMode.countdown2
    return RowReactiveCompatible.share.appConfigMode.countdown1 + RowReactiveCompatible.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class BeginMiniView: DelayMiniView {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= RowReactiveCompatible.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(kLet_senseData - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                appearDismissLocal()
    ...
    override init(frame: CGRect) {
        ...
        boneOfContentionBorder()
        //: addTapAndPanGestures()
        tincture()
        //: addNotification()
        notificationSend()

        //: if Double(RowReactiveCompatible.share.loginUserMode.mf_bean) > 0 {
        if Double(RowReactiveCompatible.share.loginUserMode.mf_bean) > 0 {
            //: setPointText(mfBean: 0.0)
            accountAdd(mfBean: 0.0)
        }
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kClickAwakeTitle.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: AddReactiveCompatible = {
        ...
        let image = UIImage.thoughtImageShowSize(color: UIColor(hex: (String(kFrameValue) + String(k_indexPendingValue.prefix(6))))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.circle(), for: .normal)
        img.setImage(image.circleContext(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(k_contentData.suffix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.collectionName(name: (String(kSectionData.prefix(5)) + "ome_sha" + String(kCurrentValue)))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        let url = DataEnvironmentReactiveCompatible.default.atType(type: .Live_countdown_bg)
        //: do {
        do {
            ...
            printLog(message: (String(k_stopData.suffix(5)) + "son error"))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        //: let lab = UILabel()
        let lab = UILabel()
        //: lab.font = UIFont.moleculeClick(fontSize: 20)
        lab.font = UIFont.moleculeClick(fontSize: 20)
        //: lab.textAlignment = .center
        lab.textAlignment = .center
        //: lab.textColor = UIColor(hex: "#FF2348")
        lab.textColor = UIColor(hex: (String(k_hideTitle.suffix(7))))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(k_infoCenterValue.suffix(5)) + String(k_cellName)))
...
extension BeginMiniView {
    /// 构建视频小窗口
    //: static func buildLiveMiniView() -> TalkingLiveMiniView {
    static func rearPush() -> BeginMiniView {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = BeginMiniView(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = SendSocketDelegate.shared.liveMiniPosition
    ...
    func appearDismissLocal() {
        //: liveStatusImgV.isHidden = !TalkingLiveManager.shared().isLive
        liveStatusImgV.isHidden = !WithNeighbourReactiveCompatible.clickEqual().isLive
...
extension BeginMiniView {
    ...
    private func notificationSend() {
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(pointchange(notification:)),
                                               selector: #selector(halogenNotification(notification:)),
                                               //: name: LIVE_USER_POINT_CHANGE,
                                               name: kLet_recordTitle,
                                               //: object: nil)
                                               object: nil)
    }

    //: @objc private func pointchange(notification: NSNotification) {
    @objc private func halogenNotification(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(kFillTitle))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        accountAdd(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func accountAdd(mfBean: Double) {
        //: let nowBean = RowReactiveCompatible.share.loginUserMode.mf_bean + mfBean
        let nowBean = RowReactiveCompatible.share.loginUserMode.mf_bean + mfBean
        //: RowReactiveCompatible.share.loginUserMode.mf_bean = nowBean
        RowReactiveCompatible.share.loginUserMode.mf_bean = nowBean
        ...
        mfBeanLB.text = "\(Int(nowBean))\((kButtonTitle.replacingOccurrences(of: "name", with: "p") + kEmptyName).localized) \(divisor)"
...
extension BeginMiniView {
    ...
    private func boneOfContentionBorder() {
