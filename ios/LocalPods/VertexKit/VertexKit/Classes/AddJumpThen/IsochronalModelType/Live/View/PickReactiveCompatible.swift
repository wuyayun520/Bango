...
fileprivate let kModelEqualValue:[UInt8] = [0x4c,0x4b,0x4c,0x51,0xd,0x46,0x4a,0x41,0x40,0x57,0x1f,0xc,0x5,0x4d,0x44,0x56,0x5,0x4b,0x4a,0x51,0x5,0x47,0x40,0x40,0x4b,0x5,0x4c,0x48,0x55,0x49,0x40,0x48,0x40,0x4b,0x51,0x40,0x41]

private func letMid(title num: UInt8) -> UInt8 {
    return num ^ 37
}

/*: "move" :*/
fileprivate let kNameActualData:[Character] = ["m","o","v","e"]
...
class PickReactiveCompatible: UIView {
	var modelTotal: Double = 8.0
	var stintArray: [AnyHashable] = []
	var errorLoadDictionary: [AnyHashable: String] = [:]
    ...
    var danumuMsgArray: [LastTransformable] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [UpgradeView] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<LastTransformable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kModelEqualValue.map{letMid(title: $0)}, encoding: .utf8)!)
    }
    ...
    func equivalent(_ danumuMsgModel: LastTransformable?) {
    ...
    @objc func scheduleAction(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(scheduleAction(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension PickReactiveCompatible: UIGestureRecognizerDelegate {
    ...
    func nameWithView(for danumuMsgModel: LastTransformable?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = UpgradeView(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (String(kNameActualData)))
    ...
    func jump(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                nameWithView(for: model, isFirst: isFirst)
...
extension PickReactiveCompatible {
    // 添加视图
