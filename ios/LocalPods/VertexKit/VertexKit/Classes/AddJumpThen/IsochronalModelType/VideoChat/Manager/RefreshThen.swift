...
/*: "mf/call/GetUserInfo" :*/
fileprivate let kTempName:String = "do fill var self blockmf/ca"
fileprivate let k_topValue:String = "make change privacy gesturetUse"

/*: "uid" :*/
fileprivate let kAllTitle:[UInt8] = [0x64,0x69,0x75]

/*: "TRTC 随机视频匹配中." :*/
fileprivate let kModelRawData:[Character] = ["T","R","T","C"," ","随","机","视","频","匹","配","\u{4e2d}","."]

/*: "TRTC  :*/
fileprivate let k_mediumData:[Character] = ["T","R","T","C"," "]

/*: "发起" :*/
fileprivate let kTopValue:[UInt8] = [0xb7,0xb5,0xe8,0x91,0x8f,0xe5]

/*: "收到" :*/
fileprivate let k_sizeAddData:String = "收到"

/*: 视频通话. logId: :*/
fileprivate let k_errorPathContent:String = "视\u{9891}通话. "
fileprivate let k_willValue:String = "logId:mode size succeed send origin"

/*: ." :*/
fileprivate let k_frameName:[Character] = ["."]

/*: "s- :*/
fileprivate let kCanData:[Character] = ["s","-"]

/*: - :*/
fileprivate let kLeadingValue:[Character] = ["-"]

/*: "TRTC 视频通话销毁. logId： :*/
fileprivate let k_scaleManagerData:String = "TRTC text sound"
fileprivate let kContainerText:[Character] = ["视","频","通","话","\u{9500}","毁","."," ","l","o","g","I","d","："]

/*: "TRTC 视频通话错误. errCode: :*/
fileprivate let k_equalData:[Character] = ["T","R","T","C"," ","\u{89c6}","频","通","话","错","\u{8bef}","."," ","e","r","r","C","o"]
fileprivate let k_modelRefreshName:[Character] = ["d","e",":"]

/*: , errMsg: :*/
fileprivate let k_viewRawTitle:String = "let click corner up, errMs"
fileprivate let k_makeKitData:[Character] = ["g",":"]

/*: "TRTC 视频通话SDK收到警告. warningCode: :*/
fileprivate let kBagText:String = "fill deal status gameTRTC 视"
fileprivate let k_allowInfoData:String = "collection label equal letK收到警告."
fileprivate let kMakeName:String = "accept viewCode:"

/*: , warningMsg: :*/
fileprivate let k_actualData:String = "size stop, war"

/*: "TRTC 视频通话进入房间. result: :*/
fileprivate let k_regularManagerTitle:String = "itemRitemC"
fileprivate let kViewStatusName:[Character] = ["间","."," ","r","e","s","u","l","t",":"]

/*: , logId： :*/
fileprivate let k_imageName:[Character] = [","," ","l","o","g","I","d","\u{ff1a}"]

/*:  加入了房间" :*/
fileprivate let kNameText:[Character] = [" ","加","入","\u{4e86}","房"]
fileprivate let kPlayerData:String = "plan"

/*:  离开了房间" :*/
fileprivate let k_valueName:String = " 离开了\u{623f}\u{95f4}"
...
/// 通话状态
//: enum VideoChatStage {
enum GenitalPhaseTelecastingChatStage {
    //: case Waiting
    case Waiting // 等待
    //: case Calling
    case Calling // 通话
...
class TimeReactiveCompatible: NSObject {
    ...
    var remoteCameraStatus = EventCameraStatus.Normal // 对方摄像头状态
}

//: class TalkingVideoChatManager: NSObject {
class RefreshThen: NSObject {
    //: var infoModel: TalkingVideoChatModel?
    var infoModel: MakeChatModel? // 视频通话model
    //: var config = TalkingVideoChatConfig()
    var config = TimeReactiveCompatible() // 视频渲染配置类
    ...
    private var trtcView: RowVideoView? // 预览view（使用商汤美颜）
...
extension RefreshThen {
    /// 开始视频速配（男）
    /// - Parameters:
    ///   - uid: 接收者的id
    ///   - completion: 回调
    //: class func req_callGetUserInfo(uid: String, completion: @escaping FinishBlock) {
    class func sharedTag(uid: String, completion: @escaping FinishBlock) {
        ...
        reqModel.requestPath = (String(kTempName.suffix(5)) + "ll/Ge" + String(k_topValue.suffix(4)) + "rInfo")
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["uid": uid]
        reqModel.params = [String(bytes: kAllTitle.reversed(), encoding: .utf8)!: uid]
...
extension RefreshThen {
    ...
    func objectWith(_ frontCamera: Bool) {
        ...
        self.chatUpPlayer(front: frontCamera)
    }

    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func cellStart(_ openCamera: Bool) {
        ...
        self.startSumeraction(isOpen: openCamera)
    }
    ...
    func sign() {
        ...
        DispatchQueue.main.async {
            ...
            self.board(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
            //: self.reloadTRTCRemotView()
            self.flushInPosition()
        }
    }
...
extension RefreshThen {
    ...
    private func chatUpPlayer(front: Bool) {
        //: guard SenseTime_Use == true else {
        guard kLet_homeContent == true else {
        ...
        self.trtcView?.bagFront(isFront: front)
    }

    /// 是否开启摄像头
    /// - Parameter isOpen: 是否开启
    //: private func setTRTCCamera(isOpen: Bool) {
    private func startSumeraction(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard kLet_homeContent == true else {
            //: if isOpen {
            if isOpen {
        ...
        self.trtcView?.addUpFill(isOpen: isOpen)
...
extension RefreshThen {
    ...
    func nameMake() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(showKey(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(viewKind(notif:)),
    ...
    @objc private func showKey(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func viewKind(notif: Notification?) {
        //: if notif == nil {
    ...
    @objc private func old(notif: Notification) {
    ...
    @objc func party(notif: Notification) {
...
extension RefreshThen {
    ...
    func toClick(isMatch: Bool = false) {
        //: if isMatch {
        if isMatch {
            //: JumpLogTool.lineLoad(msg: "TRTC 随机视频匹配中.")
            JumpLogTool.lineLoad(msg: (String(kModelRawData)))
            //: } else {
        } else {
            //: JumpLogTool.lineLoad(msg: "TRTC \((infoModel?.videoStage == .Waiting) ? "发起":"收到")视频通话. logId:\(infoModel?.logId ?? 0).")
            JumpLogTool.lineLoad(msg: (String(k_mediumData)) + "\((infoModel?.videoStage == .Waiting) ? "发起" : "收到")" + (k_errorPathContent + String(k_willValue.prefix(6))) + "\(infoModel?.logId ?? 0).")
        }
        //: if self.infoModel?.videoStage == .Waiting {
        if self.infoModel?.videoStage == .Waiting {
            //: startTRTCPreview(smallWindow: false)
            board(smallWindow: false)
    ...
    func board(smallWindow: Bool) {
        ...
        if self.trtcView == nil {
            ...
            self.trtcView = RowVideoView()
        }
        //: self.trtcView?.startPreView(isSmallWindow: smallWindow)
        self.trtcView?.thoughtImage(isSmallWindow: smallWindow)
        //: if smallWindow {
    ...
    private func fullMoon() {
        //: JumpLogTool.lineLoad(msg: "TRTC 视频通话销毁. logId：\(infoModel?.logId ?? 0).")
        JumpLogTool.lineLoad(msg: (String(k_scaleManagerData.prefix(5)) + String(kContainerText)) + "\(infoModel?.logId ?? 0).")
...
extension RefreshThen: TRTCCloudDelegate {
    //: func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo: [AnyHashable: Any]?) {
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        //: JumpLogTool.lineLoad(msg: "TRTC 视频通话错误. errCode:\(errCode), errMsg:\(String(describing: errMsg)).")
        JumpLogTool.lineLoad(msg: (String(k_equalData) + String(k_modelRefreshName)) + "\(errCode)" + (String(k_viewRawTitle.suffix(7)) + String(k_makeKitData)) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        //: JumpLogTool.lineLoad(msg: "TRTC 视频通话SDK收到警告. warningCode:\(warningCode), warningMsg:\(String(describing: warningMsg)).")
        JumpLogTool.lineLoad(msg: (String(kBagText.suffix(6)) + "频通话SD" + String(k_allowInfoData.suffix(6)) + " warning" + String(kMakeName.suffix(5))) + "\(warningCode)" + (String(k_actualData.suffix(5)) + "ningMsg:") + "\(String(describing: warningMsg)).")
        ...
        if self.config.remoteCameraStatus == .Normal {
            ...
            NotificationCenter.default.post(name: kLet_domainName, object: self)
        }
    }

    //: func onEnterRoom(_ result: Int) {
    func onEnterRoom(_ result: Int) {
        //: JumpLogTool.lineLoad(msg: "TRTC 视频通话进入房间. result:\(result), logId：\(infoModel?.logId ?? 0).")
        JumpLogTool.lineLoad(msg: (k_regularManagerTitle.replacingOccurrences(of: "item", with: "T") + " 视频\u{901a}话进入房" + String(kViewStatusName)) + "\(result)" + (String(k_imageName)) + "\(infoModel?.logId ?? 0).")
        //: if result < 0 {
        if result < 0 { // 加入失败
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (String(kNameText) + kPlayerData.replacingOccurrences(of: "plan", with: "间")))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason _: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (k_valueName))
    }
    ...
    func onUserVideoAvailable(_ userId: String, available: Bool) {
        //: guard userId != RowReactiveCompatible.share.loginUserMode.userID else { return }
        guard userId != RowReactiveCompatible.share.loginUserMode.userID else { return }
