...
/*: "call_bgm" :*/
fileprivate let k_styleName:String = "call_bgmrefuse model self"

/*: "call_response_bgm" :*/
fileprivate let k_headValue:[Character] = ["c","a","l","l","_","r","e","s","p","o","n","s","e","_","b"]
fileprivate let k_indexTitle:[Character] = ["g","m"]
...
class SphericPlayListener: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = DelayTextConvertible.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func titleType(type: DelayTextConvertible) -> SphericPlayListener {
        //: let player = TalkingCallBGMPlayer()
        let player = SphericPlayListener()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.blockWith()
...
extension SphericPlayListener {
    ...
    private func mobile(notify: Notification) {
    ...
    private func titleServer(notify: Notification) {
    ...
    private func blockWith() {
        ...
        var fileName = (String(k_styleName.prefix(8)))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(k_headValue) + String(k_indexTitle))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = DataEnvironmentReactiveCompatible.default.deciding(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension SphericPlayListener: TXVodPlayListener {
