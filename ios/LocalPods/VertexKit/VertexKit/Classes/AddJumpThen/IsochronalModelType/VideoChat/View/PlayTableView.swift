...
fileprivate let kGameCurveData:[UInt8] = [0x56,0x51,0x56,0x4b,0x17,0x5c,0x50,0x5b,0x5a,0x4d,0x5,0x16,0x1f,0x57,0x5e,0x4c,0x1f,0x51,0x50,0x4b,0x1f,0x5d,0x5a,0x5a,0x51,0x1f,0x56,0x52,0x4f,0x53,0x5a,0x52,0x5a,0x51,0x4b,0x5a,0x5b]

private func baseballPlayer(video num: UInt8) -> UInt8 {
    return num ^ 63
}
...
@objc protocol DatabaseViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func indexRange()
...
class PlayTableView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(withFront))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kGameCurveData.map{baseballPlayer(video: $0)}, encoding: .utf8)!)
    }
...
extension PlayTableView {
    ...
    func edit() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(timer), userInfo: nil, repeats: true)
    }
...
extension PlayTableView {
    ...
    func modifyModel(msgModel: GiftModelType?) {
    ...
    func sumShow() {
        ...
        if !self.inPending {
            ...
            counternalRepresentation()
        }
    }

    /** 追加数据源 */
    //: func appendAndScrollToBottom() {
    func counternalRepresentation() {
        ...
        if AllHeight > kLet_buttonMarginValue {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < kLet_buttonMarginValue {
                ...
                self.msgTableView.height = kLet_buttonMarginValue
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = kLet_buttonMarginValue - AllHeight
        ...
        giftLoad(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func giftLoad(animated: Bool) {
    ...
    func unfinished(inPending: Bool) {
...
extension PlayTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func outsideCamera(_ scrollView: UIScrollView) {
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(kLet_listDisplayKeyData)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = MusicViewCell.crush(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension PlayTableView {
    ...
    func counterval() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: kLet_messageName, height: kLet_buttonMarginValue)
    }
}
