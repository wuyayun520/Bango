...
fileprivate let k_styleContent:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class DelayMiniView: UIView {
	var imageClose: Bool = true
	var videoCount: Double = -94.6
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_styleContent.reversed(), encoding: .utf8)!)
    }
    ...
    override func willMove(toSuperview _: UIView?) {
        ...
        SendSocketDelegate.shared.containerView?.isHidden = false
    ...
    func rowData() {
        ...
        UIView.animate(withDuration: 0.3, delay: 2, options: .curveEaseOut) {
            ...
            let x = (self.center.x <= kLet_errData * 0.5) ? -self.width : kLet_errData + self.width
            ...
            SendSocketDelegate.shared.containerView?.isHidden = true
        }
    }
}
...
extension DelayMiniView {
    /// 添加点击手势
    //: @discardableResult
    @discardableResult
    //: func addTapGesture() -> UITapGestureRecognizer {
    func upGesture() -> UITapGestureRecognizer {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(self.tapGestureEvent(sender:)))
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.groupAction(sender:)))
    ...
    func tincture() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(self.oeilDeBoeufText(sender:)))
        //: let tap = self.addTapGesture()
        let tap = self.upGesture()
    ...
    @objc func oeilDeBoeufText(sender: UIPanGestureRecognizer) {
        ...
        centerX = min(kLet_errData - self.width * 0.5, centerX)
        //: centerY = max(self.height*0.5, centerY)
        centerY = max(self.height * 0.5, centerY)
        //: centerY = min(ScreenHeight-self.height*0.5, centerY)
        centerY = min(kLet_nameValue - self.height * 0.5, centerY)
        ...
        if sender.state == .ended {
            ...
            var endCenterX = kLet_errData - self.width * 0.5
            //: var recordX = ScreenWidth
            var recordX = kLet_errData
            //: if centerX <= ScreenWidth*0.5 {
            if centerX <= kLet_errData * 0.5 {
            ...
            SendSocketDelegate.shared.videoMiniPosition = CGPoint(x: recordX, y: recordY)
        }
    }

    /// 点击手势
    //: @objc private func tapGestureEvent(sender: UIPanGestureRecognizer) {
    @objc private func groupAction(sender _: UIPanGestureRecognizer) {
