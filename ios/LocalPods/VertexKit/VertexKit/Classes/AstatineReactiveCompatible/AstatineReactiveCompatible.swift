...
/*: "&sr=6" :*/
fileprivate let k_modeRequestInData:[Character] = ["&","s","r","=","6"]
...
enum ItemConvertible: String, CaseIterable {
    ...
    case MethodType_changeMenuButton = "changeMenuButton" // 更改按钮
    //: case MethodType_refreshCoin                   = "refreshCoin"              // 【paypal】刷新金币
    case MethodType_refreshCoin = "refreshCoin" // 【paypal】刷新金币
...
class AstatineReactiveCompatible: NSObject {
    //: class func getFullUrl(urlType: WebUrlType)-> String {
    class func typeOn(urlType: LastSequence) -> String {
        //: var url = RowReactiveCompatible.share.appConfigMode.urlH5Domain
        var url = RowReactiveCompatible.share.appConfigMode.urlH5Domain
        //: switch urlType {
        switch urlType {
        //: case .RechargeFullPage:
        case .RechargeFullPage:
            //: url.append(RowReactiveCompatible.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : RowReactiveCompatible.share.appUserConfigMode.fullRechargePage)
            url.append(RowReactiveCompatible.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : RowReactiveCompatible.share.appUserConfigMode.fullRechargePage)

        //: break
        //: case .RechargeHalfPage:
        case .RechargeHalfPage:
            //: url.append(RowReactiveCompatible.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : RowReactiveCompatible.share.appUserConfigMode.halfRechargePage)
            url.append(RowReactiveCompatible.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : RowReactiveCompatible.share.appUserConfigMode.halfRechargePage)

        //: case .RechargeToVideoInitivHalfPage:
        case .RechargeToVideoInitivHalfPage:
            //: let newstr = RowReactiveCompatible.share.appUserConfigMode.halfRechargePage.appending("&sr=6")
            let newstr = RowReactiveCompatible.share.appUserConfigMode.halfRechargePage.appending((String(k_modeRequestInData)))
            //: url.append(RowReactiveCompatible.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
            url.append(RowReactiveCompatible.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
        //: default:
        default:
