...
/*: "Finish" :*/
fileprivate let k_indexAddTitle:String = "Finishimport equal"

/*: "Cancel" :*/
fileprivate let k_viewName:[Character] = ["C","a","n","c","e","l"]

/*: "btn_photo_choice_pre" :*/
fileprivate let k_destroyValue:[Character] = ["b","t","n","_","p","h","o","t","o","_"]
fileprivate let k_modelEqualData:String = "CHOIC"

/*: "btn_photo_add_nor" :*/
fileprivate let kViewData:String = "btn_phview info custom"
fileprivate let k_bottomMediaLetValue:String = "redtred"
fileprivate let kArrayData:String = "_add_norself self transaction"

/*: "( :*/
fileprivate let k_playerValue:String = "model"

/*: )" :*/
fileprivate let k_keyName:String = "path"

/*: "/tmp" :*/
fileprivate let k_sendValue:[Character] = ["/","t","m","p"]

/*: "Export session failed" :*/
fileprivate let k_logTitle:String = "Export smoment status"
fileprivate let k_cornerName:[Character] = ["e","s","s"]
fileprivate let kDataValue:String = "ion self view camera"

/*: "Export canceled" :*/
fileprivate let kAddName:String = "at top indexExpo"
fileprivate let kVoiceTargetData:String = "CELED"

/*: "Successful!" :*/
fileprivate let k_labData:[Character] = ["S","u","c","c","e","s","s","f","u","l","!"]

/*: "Unsupported video formats" :*/
fileprivate let k_sData:String = "at index right viewUnsup"
fileprivate let kUserName:String = "PORT"
fileprivate let k_labelData:[Character] = ["e","d"," ","v","i","d","e","o"," ","f","o","r","m","a","t","s"]

/*: "yyyy-MM-dd-HH:mm:ss-SSS" :*/
fileprivate let kAwakeAppData:String = "showshow"
fileprivate let kCellTitle:String = "dd-HH:background name section not"
fileprivate let k_contentUserValue:String = "private model-SSS"

/*: "/tmp/video- :*/
fileprivate let k_frameValue:String = "label text return/tmp/"
fileprivate let kUniversalData:[Character] = ["v","i","d","e","o","-"]

/*: - :*/
fileprivate let kConversationAboutName:String = "-"

/*: .mp4" :*/
fileprivate let k_inputValue:[Character] = [".","m","p","4"]
...
class ToolThen: NSObject {
    ...
    class func tied(allowImgCrop: Bool = false, needCircleCrop: Bool = false, maxCount: Int, allowPhoto: Bool, allowVideo: Bool) -> TZImagePickerController {
...
//        imagePicker?.preferredLanguage = "en"
        ...
        if allowImgCrop {
            ...
            var height = kLet_errData * 488 / 375
            ...
            imagePicker?.cropRect = CGRect(x: 0, y: (kLet_nameValue - height) / 2, width: kLet_errData, height: height)
        }
        //: if allowVideo {
        if allowVideo {
        ...
        imagePicker?.doneBtnTitleStr = (String(k_indexAddTitle.prefix(6))).localized
        //: imagePicker?.cancelBtnTitleStr = "Cancel".localized
        imagePicker?.cancelBtnTitleStr = (String(k_viewName)).localized

        //: imagePicker?.barItemTextFont = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        imagePicker?.barItemTextFont = UIFont.ofAndSize(type: .Regular, fontSize: 15)
        //: imagePicker?.barItemTextColor = UIColor.appValueColor()
        imagePicker?.barItemTextColor = UIColor.appPath()
        ...
        imagePicker?.naviTitleFont = UIFont.ofAndSize(type: .Medium, fontSize: 17)
        //: imagePicker?.naviTitleColor = UIColor.appTitleColor()
        imagePicker?.naviTitleColor = UIColor.dropMultiView()
        ...
        imagePicker?.iconThemeColor = UIColor.cellColor()

        //: imagePicker?.allowPickingOriginalPhoto = false
        imagePicker?.allowPickingOriginalPhoto = false
        //: imagePicker?.photoSelImage = UIImage.collectionName(name: "btn_photo_choice_pre")
        imagePicker?.photoSelImage = UIImage.collectionName(name: (String(k_destroyValue) + k_modelEqualData.lowercased() + "e_pre"))
        //: imagePicker?.takePictureImage = UIImage.collectionName(name: "btn_photo_add_nor")
        imagePicker?.takePictureImage = UIImage.collectionName(name: (String(kViewData.prefix(6)) + k_bottomMediaLetValue.replacingOccurrences(of: "red", with: "o") + String(kArrayData.prefix(8))))
        ...
        imagePicker?.photoPickerPageDidRefreshStateBlock = { (_: UICollectionView?,
                ...
                doneButton?.titleLabel?.font = UIFont.ofAndSize(type: .Medium, fontSize: 14)
                ...
                if text.count > 0 && Int(text)! > 0 {
                    ...
                    doneButton?.setTitle((String(k_indexAddTitle.prefix(6))).localized + "(\(text))", for: .normal)
                    //: } else {
                } else {
                    //: doneButton?.setTitle("Finish".localized, for: .normal)
                    doneButton?.setTitle((String(k_indexAddTitle.prefix(6))).localized, for: .normal)
                }
                //: return
        }
        ...
        imagePicker?.photoPickerPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                doneButton?.backgroundColor = UIColor.cellColor()
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
        ...
        imagePicker?.photoPreviewPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                selectButton?.frame = CGRect(x: kLet_errData - 5 - (selectButton?.frame.size.width)!, y: (selectButton?.frame.origin.y)!, width: (selectButton?.frame.size.width)!, height: (selectButton?.frame.size.height)!)
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
    ...
    class func businessIndex(asset: PHAsset?, completion: @escaping (_ filePath: URL?) -> Void) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: asset!, options: options) { (asset: AVAsset?, _: AVAudioMix?, _) in
            ...
            if asset!.isKind(of: AVURLAsset.self) {
                ...
                if !FileManager.default.fileExists(atPath: NSHomeDirectory().appending((String(k_sendValue)))) {
                    //: do {
                    do {
                        //: try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending("/tmp"), withIntermediateDirectories: true)
                        try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending((String(k_sendValue))), withIntermediateDirectories: true)
                        //: } catch {
                    } catch {}
                }
                ...
                exportSession?.exportAsynchronously(completionHandler: {
                    ...
                    case .failed:
                        ...
                        printLog(message: (String(k_logTitle.prefix(8)) + String(k_cornerName) + String(kDataValue.prefix(4)) + "failed"))
                    //: case .cancelled:
                    case .cancelled:
                        //: printLog(message: "Export canceled")
                        printLog(message: (String(kAddName.suffix(4)) + "rt can" + kVoiceTargetData.lowercased()))
                    //: case .completed:
                    case .completed:
                        //: printLog(message: "Successful!")
                        printLog(message: (String(k_labData)))
                        //: DispatchQueue.main.async {
                        DispatchQueue.main.async {
                ...
                printLog(message: (String(k_sData.suffix(5)) + kUserName.lowercased() + String(k_labelData)))
            }
        }
    }
    ...
    private class func keyFromPath() -> String {
        //: let formater = DateFormatter()
        let formater = DateFormatter()
        //: formater.dateFormat = "yyyy-MM-dd-HH:mm:ss-SSS"
        formater.dateFormat = (kAwakeAppData.replacingOccurrences(of: "show", with: "yy") + "-MM-" + String(kCellTitle.prefix(6)) + "mm:ss" + String(k_contentUserValue.suffix(4)))
        //: let outputPath = NSHomeDirectory().appending("/tmp/video-\(formater.string(from: Date()))-\(Int.random(in: 0...10000000)).mp4")
        let outputPath = NSHomeDirectory().appending((String(k_frameValue.suffix(5)) + String(kUniversalData)) + "\(formater.string(from: Date()))-\(Int.random(in: 0 ... 10_000_000))" + (String(k_inputValue)))
        //: return outputPath
        return outputPath
    }
}
