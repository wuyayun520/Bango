...
fileprivate let k_aspectLocationName:[UInt8] = [0xc3,0xc4,0xc3,0xde,0x82,0xc9,0xc5,0xce,0xcf,0xd8,0x90,0x83,0x8a,0xc2,0xcb,0xd9,0x8a,0xc4,0xc5,0xde,0x8a,0xc8,0xcf,0xcf,0xc4,0x8a,0xc3,0xc7,0xda,0xc6,0xcf,0xc7,0xcf,0xc4,0xde,0xcf,0xce]

private func picFatal(make num: UInt8) -> UInt8 {
    return num ^ 170
}
...
typealias SlidingHandler = (_ direction: MakeSlideDirection) -> Void
...
class EditThen: UIView {
    ...
    var cropStart: CGFloat {
        ...
        let offset = overlay.startOffset - (kLet_needTitle - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_aspectLocationName.map{picFatal(make: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.listener(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: TakeCropOverlay = {
        ...
        let overlay = TakeCropOverlay(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension EditThen {
    ...
    func exaggeratePostion() {
        ...
        self.overlay.imageQuit(startX: kLet_needTitle - x)
    }
    ...
    private func listener(point: CGFloat) -> UIImage {
...
extension EditThen: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = kLet_needTitle - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.needSize(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.needSize(isShow: true)
    }
...
extension EditThen {
    ...
    private func subviewsColor() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(kLet_needTitle)
