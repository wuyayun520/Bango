...
fileprivate let kTitleValue:[UInt8] = [0x5f,0x58,0x5f,0x42,0x1e,0x55,0x59,0x52,0x53,0x44,0xc,0x1f,0x16,0x5e,0x57,0x45,0x16,0x58,0x59,0x42,0x16,0x54,0x53,0x53,0x58,0x16,0x5f,0x5b,0x46,0x5a,0x53,0x5b,0x53,0x58,0x42,0x53,0x52]

private func youQuantity(multi num: UInt8) -> UInt8 {
    return num ^ 54
}

/*: "FFD855" :*/
fileprivate let k_rawData:String = "FFDbottom55"
...
class TakeCropOverlay: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - kLet_maxName
                ...
                self.notRequestPosition()
                //: self.refreshSliderValidScope(startX: self.videoStartX)
                self.imageQuit(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kTitleValue.map{youQuantity(multi: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (k_rawData.replacingOccurrences(of: "bottom", with: "8")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: kLet_needTitle, y: 0, width: kLet_maxName, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - kLet_maxName, y: 0, width: kLet_maxName, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.viewSearch(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: kLet_needTitle, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.viewSearch(0, 0, 0, 0.6)
...
extension TakeCropOverlay {
    ...
    func imageQuit(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - kLet_maxName,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kLet_maxName)
    }
}
...
extension TakeCropOverlay {
    ...
    @objc func musterOut(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.imageQuit(startX: videoStartX)
    ...
    @objc func pastExecute(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension TakeCropOverlay {
    ...
    private func moreDate() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(musterOut(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(pastExecute(gesture:)))
    ...
    func modifyXpositionClick(_ xposition: CGFloat) {
    ...
    private func notRequestPosition() {
