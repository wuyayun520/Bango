...
fileprivate let k_topMakeName:[UInt8] = [0xd2,0xd5,0xd2,0xcf,0x93,0xd8,0xd4,0xdf,0xde,0xc9,0x81,0x92,0x9b,0xd3,0xda,0xc8,0x9b,0xd5,0xd4,0xcf,0x9b,0xd9,0xde,0xde,0xd5,0x9b,0xd2,0xd6,0xcb,0xd7,0xde,0xd6,0xde,0xd5,0xcf,0xde,0xdf]

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let k_actionData:String = "btnJQbreference end video else"
fileprivate let k_requestColorValue:String = "tihw_value succeed"
fileprivate let k_messageName:String = "indexadow"

/*: "Edit" :*/
fileprivate let k_leadingName:String = "Editself self any"

/*: "Finish" :*/
fileprivate let kSignTitle:String = "Finisequal request case table for"
fileprivate let kEmptyName:String = "color"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let k_partySendData:[Character] = ["b","t","n","Y","D","o","2","A","S","_","o","e"]
fileprivate let kNameValue:String = "return mask text begindiv_play"

/*: "Cancel" :*/
fileprivate let kListValue:[Character] = ["C","a","n","c","e","l"]

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let k_sendTitle:[Character] = ["b","g","Q","k","R","T","b","F","_","o","e","d","i","v"]
fileprivate let k_equalData:[Character] = ["_","t","o","a","s","t"]

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let kLeadingValue:[UInt8] = [0x47,0x7b,0x76,0x33,0x65,0x7a,0x77,0x76,0x7c,0x33,0x7f,0x76,0x7d,0x74,0x67,0x7b,0x33,0x60,0x7b,0x7c,0x66,0x7f,0x77,0x33,0x71,0x76,0x33,0x36,0x53,0x3e,0x36,0x53,0x33,0x60,0x76,0x70,0x7c,0x7d,0x77,0x60,0x3d,0x33,0x43,0x7f,0x76,0x72,0x60,0x76,0x33,0x7a,0x7d,0x67,0x76,0x61,0x70,0x76,0x63,0x67,0x33,0x7a,0x67]
...
typealias VideoEditBlock = (_ videoInfo: RestoreMeasurable?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let kLet_purchaseValue: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class TvThen: RecordReactiveCompatible {
	var backgroundDictionary: [AnyHashable: String] = [:]
	var myDictionary: [AnyHashable: String] = [:]
    ...
    private var slider: EditThen?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = RestoreMeasurable() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = DelayCurrentEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if SendSocketDelegate.shared.isTalking == true { // 正在音视频通话，静音播放
    ...
    init(style: DelayCurrentEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (kLet_errData - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_topMakeName.map{$0^187}, encoding: .utf8)!)
    }
    ...
    override func viewDidLoad() {
        ...
        self.cellMoveConstraint()
        //: self.bindInteraction()
        self.theSumeraction()
        //: self.func__addNotification()
        self.funcInNotificationRemove()
	}
    ...
    private lazy var backBtn: UIButton = {
        ...
        let v = UIButton(type: .custom)
        //: v.setImage(UIImage.collectionName(name: "btnJQbEAB_etihw_kcab_shadow"), for: .normal)
        v.setImage(UIImage.collectionName(name: (String(k_actionData.prefix(6)) + "EAB_e" + String(k_requestColorValue.prefix(5)) + "kcab_s" + k_messageName.replacingOccurrences(of: "index", with: "h"))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = TextLanguageManager.shared.direction == .rightToLeft ? .right : .left
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(k_leadingName.prefix(4))).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.cellColor(), for: .normal)
...
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(kSignTitle.prefix(5)) + kEmptyName.replacingOccurrences(of: "color", with: "h")).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.moleculeClick(fontSize: 14)
        v.titleLabel?.font = UIFont.moleculeClick(fontSize: 14)
        ...
        v.backgroundColor = UIColor.cellColor()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(refer), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        //: let v = UIButton(type: .custom)
        let v = UIButton(type: .custom)
        //: v.setImage(UIImage.collectionName(name: "btnYDo2AS_oediv_play"), for: .normal)
        v.setImage(UIImage.collectionName(name: (String(k_partySendData) + String(kNameValue.suffix(8)))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(upTo(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(kListValue)).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(cellOf), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        //: let v = UIImageView(image: UIImage.collectionName(name: "bgQkRTbF_oediv_toast"))
        let v = UIImageView(image: UIImage.collectionName(name: (String(k_sendTitle) + String(k_equalData))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: kLeadingValue.map{$0^19}, encoding: .utf8)!.roundName(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: DataForefrontReactiveCompatible = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = DataForefrontReactiveCompatible(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension TvThen {
    ...
    private func hiddenTo(animated _: Bool = true) {
    ...
    @objc func touchFor() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: kLet_errData - 2 * 26, height: kLet_nameValue - 46 - 127 - kLet_itemData)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = kLet_errData / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: kLet_nameValue, width: kLet_errData, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = EditThen(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: kLet_purchaseValue)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - kLet_itemData)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: MakeSlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func refer() {
        ...
        if self.videoEditHandler != nil {
            ...
            IndexSignatureProvider.shared.buttonHandler(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.hiddenTo()
    ...
    @objc func upTo(sender: UIButton) {
...
extension TvThen {
    ...
    private func pathDown() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(buttonSuccess), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.modifyXpositionClick(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func cellMoveConstraint() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + kLet_itemData)
        }
    ...
    private func funcInNotificationRemove() {
        // 监听应用进入后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(didEnterBackground),
                                               selector: #selector(beyondModel),
