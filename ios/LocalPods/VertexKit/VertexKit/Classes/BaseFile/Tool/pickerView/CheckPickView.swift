...
/*: "Select a date" :*/
fileprivate let kSectionName:String = "Secreate self self"
fileprivate let k_backgroundCellReplyTitle:String = "string cell lengthlect "

/*: M" :*/
fileprivate let k_sessionKitTitle:[Character] = ["M"]

/*: D" :*/
fileprivate let kPlayerData:String = "name"

/*: Y" :*/
fileprivate let k_makeData:[Character] = ["Y"]
...
protocol BagViewDelegate: NSObjectProtocol {
...
class CheckPickView: WithDetailView, UIPickerViewDataSource, UIPickerViewDelegate {
	var earTitle: String = "service"
	var cosArray: [AnyHashable] = []
	var equalDictionary: [AnyHashable: String] = [:]
	var adName: String = "all"
	var replyDirectionArray: [AnyHashable] = []
	var scheduleDictionary: [AnyHashable: String] = [:]
	var showContent: String = "pin"
    ...
    override func appearAcrossView() {
        //: super.initPickView()
        super.appearAcrossView()

        //: titleString = "Select a date".localized
        titleString = (String(kSectionName.prefix(2)) + String(k_backgroundCellReplyTitle.suffix(5)) + "a date").localized
        ...
        inputWill(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func inputWill(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func err(button: UIButton) {
        //: super.confirmClick(button: button)
        super.err(button: button)
        //: refreshPickViewData()
        eigenvalueOfASquareMatrixMagnitude()

        //: if let day = selectDay {
        if let day = selectDay {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: day)
            delegate?.growth(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.growth(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return addMonth(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func addMonth(year: Int, month: Int) -> Int {
    ...
    func eigenvalueOfASquareMatrixMagnitude() {
