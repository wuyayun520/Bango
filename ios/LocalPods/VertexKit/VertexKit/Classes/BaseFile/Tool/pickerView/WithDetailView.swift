...
fileprivate let kColorErrTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Cancel" :*/
fileprivate let kViewOriginText:[Character] = ["C","a","n","c","e"]
fileprivate let kEffectValue:String = "frame"

/*: "Sure" :*/
fileprivate let k_lengthData:String = "Surestring back data"
...
class WithDetailView: UIView {
	var searchedDoingText: String = "bean"
	var withArray: [AnyHashable] = []
	var countContent: String = "make"
	var tapArray: [AnyHashable] = []
	var upName: String = "with"
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kColorErrTitle.reversed(), encoding: .utf8)!)
    }
    ...
    private func collectionView() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(levelParadigmPan))
        ...
        cancelButton.setTitle((String(kViewOriginText) + kEffectValue.replacingOccurrences(of: "frame", with: "l")).localized, for: .normal)
        //: cancelButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        cancelButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        //: cancelButton.setTitleColor(UIColor.black, for: .normal)
        cancelButton.setTitleColor(UIColor.black, for: .normal)
        //: cancelButton.addTarget(self, action: #selector(cancelClick(button:)), for: .touchUpInside)
        cancelButton.addTarget(self, action: #selector(nearRow(button:)), for: .touchUpInside)
        ...
        confirmButton.setTitle((String(k_lengthData.prefix(4))).localized, for: .normal)
        //: confirmButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        confirmButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        //: confirmButton.setTitleColor(UIColor.black, for: .normal)
        confirmButton.setTitleColor(UIColor.black, for: .normal)
        //: confirmButton.addTarget(self, action: #selector(confirmClick(button:)), for: .touchUpInside)
        confirmButton.addTarget(self, action: #selector(err(button:)), for: .touchUpInside)
    ...
    @objc func nearRow(button _: UIButton) {
        //: dismissView()
        levelParadigmPan()
	}

    //: @objc func confirmClick(button: UIButton) {
    @objc func err(button _: UIButton) {
        //: dismissView()
        levelParadigmPan()
    }
