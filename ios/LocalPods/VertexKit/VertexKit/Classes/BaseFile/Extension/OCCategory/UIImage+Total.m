...
- (UIImage *)lock {
    ...
    return [UIImage imageWithData:[self compressed]];
}
...
- (UIImage *)toCompres:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self scale:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self file:size];
...
- (CGSize)scale:(Boolean)isSession {
...
+ (NSData *)toSize:(UIImage *)image gift:(NSInteger)fileSize{
    ...
    NSData *outIMGData = [thumIMG of:(fileSize*1024)];
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG totalimate:scalesize strip:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG of:(fileSize*1024)];
    }
...
- (UIImage *)totalimate:(CGSize)targetSize strip:(BOOL)isScale{
