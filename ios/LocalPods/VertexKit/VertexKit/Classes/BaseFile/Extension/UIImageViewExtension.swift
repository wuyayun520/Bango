...
/*: "add_header_cookies" :*/
fileprivate let k_maxText:[Character] = ["a","d","d","_","h","e","a","d","e","r","_","c","o","o","k"]
fileprivate let kManagerTitle:String = "isources"
...
public extension UIImageView {
    ...
    @objc func followOut(urlStr: String,
    ...
    internal func stopDown(urlStr: String,
                           //: placeImg: UIImage = UIImage.placeImgSquare(),
                           placeImg: UIImage = UIImage.enableContent(),
    ...
    @objc func pastMoment(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: kLet_domainData.constraintView(), with: kLet_intimateData)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.partyImage())
    }
    ...
    @objc func every(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: GiftViewReactiveCompatible.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(k_maxText) + kManagerTitle.replacingOccurrences(of: "source", with: "e")))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.enableContent(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
