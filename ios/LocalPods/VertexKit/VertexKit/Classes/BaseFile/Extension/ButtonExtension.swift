...
/*: "recharge" :*/
fileprivate let k_firstAppValue:String = "RECHARGE"

/*: "recharge_ :*/
fileprivate let kMediumValue:String = "recharge_normal frame"

/*: ".png" :*/
fileprivate let k_inputGiftName:String = ".pngblock section height let"

/*: "_ :*/
fileprivate let kModeName:[Character] = ["_"]

/*: .png" :*/
fileprivate let kMoleculeText:[Character] = [".","p","n","g"]

/*: ".jpg" :*/
fileprivate let kLengthDoingData:String = ".jpgback if"

/*: .jpg" :*/
fileprivate let kImageContent:[Character] = [".","j","p","g"]
...
enum MentalImageCount: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class AddReactiveCompatible: UIButton {
	var viewTotal: Int = 76
	var usufructuaryCount: Int = 94
	var errorCount: Int = 23
    ...
    var imageAlignment: MentalImageCount = .left
...
extension UIButton {
    ...
    func asCornerText(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        upended(interval: interval, topOffset: 0)
    }
    ...
    func upended(interval: CGFloat, topOffset: CGFloat) {
    ...
    func steepWithEnableset(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func australianState(color: UIColor, forState: UIControl.State) {
    ...
    func bottomCurrent(urlStr: String,
                       //: placeImg: UIImage? = UIImage.placeImgSquare(),
                       placeImg: UIImage? = UIImage.enableContent(),
                       ...
                       loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: kLet_domainData.constraintView(), with: kLet_intimateData)
    ...
    func outWhen(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((k_firstAppValue.lowercased())) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (k_firstAppValue.lowercased()), with: (String(kMediumValue.prefix(9))) + "\(kLet_inviteValue)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.bottomCurrent(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(k_inputGiftName.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(k_inputGiftName.prefix(4))), with: "_\(kLet_inviteValue)" + (String(kMoleculeText)))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(kLengthDoingData.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(kLengthDoingData.prefix(4))), with: "_\(kLet_inviteValue)" + (String(kImageContent)))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.bottomCurrent(urlStr: newUrl, placeImg: nil)
    }
