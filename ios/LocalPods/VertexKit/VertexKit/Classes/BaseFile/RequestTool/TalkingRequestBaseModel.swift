...
/*: "http://stat. :*/
fileprivate let kAddName:String = "http:handle background view cell cell"

/*: .com" :*/
fileprivate let kCombinedTimeSharedText:[Character] = [".","c","o","m"]

/*: "http://" :*/
fileprivate let k_trackDataTitle:String = "http://message status after layer"

/*: "https://" :*/
fileprivate let k_viewValue:String = "with let appear being equalhttps://"

/*: "http://m. :*/
fileprivate let kStatusValueContent:String = "current lab player managerhttp://m"
fileprivate let k_photoData:String = "official"

/*: "http:// :*/
fileprivate let kPlayerData:String = "let devicehttp://"

/*: "-1" :*/
fileprivate let k_countText:String = "-1"

/*: "3,90" :*/
fileprivate let k_textTitle:[Character] = ["3",",","9","0"]

/*: "1990-01-01" :*/
fileprivate let kColorTitle:[Character] = ["1","9","9","0","-","0","1","-","0","1"]

/*: "MM-dd-yyyy" :*/
fileprivate let kMultiName:[Character] = ["M","M","-","d"]
fileprivate let k_addText:[Character] = ["d","-","y","y","y","y"]

/*: "id" :*/
fileprivate let k_iconData:[Character] = ["i","d"]
...
//: class TalkingMakePtolemaicSystemThen: NSObject {
class TalkingMakePtolemaicSystemThen: NSObject {
    ...
    var requestType: ChangeRequestType?
    ...
    override init() {
        ...
        self.requestServer = GiftViewReactiveCompatible.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func beatificationTo() -> Bool {
...
public struct CurrentErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct EqualTransformable: HandyJSON {
    ...
    var CSConfig = MeanSunModelType() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(kAddName.prefix(5)) + "//stat.") + "\(kLet_intimateData)" + (String(kCombinedTimeSharedText))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(k_trackDataTitle.prefix(7)))) && !statDomain.hasPrefix((String(k_viewValue.suffix(8)))) {
                //: return "http://".appending(statDomain)
                return (String(k_trackDataTitle.prefix(7))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(kStatusValueContent.suffix(8)) + k_photoData.replacingOccurrences(of: "official", with: ".")) + "\(kLet_intimateData)" + (String(kCombinedTimeSharedText))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(kPlayerData.suffix(7))) + "\(urlDomain)"
        }
    }
...
public class OverdoHandyJSON: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [WithModelType] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [WithModelType] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [WithModelType] = [] // 语音收费设置
    ...
    var vipSetting = [CuttingEdgeModelType]() // vip皮肤
    ...
    var tabPageBanner = [EventMeasurable]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = DataModelType() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class VanguardUserModel: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [HarmonyEquatable]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [HarmonyEquatable]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var callPopupSetting = (String(k_textTitle)) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.insiderInformationFormat(self.birthday ?? (String(kColorTitle)), dateFormat: (String(kMultiName) + String(k_addText)))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.getDisplay().year else { return "0" }
...
struct ColorMeasurable: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct HarmonyEquatable: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func large(compareTo: HarmonyEquatable) -> Bool {
        //: return
        return
...
struct TitleMeasurable: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [BugOutMeasurable]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [BugOutMeasurable]!
}

//: struct UserTagTypeModel: HandyJSON {
struct BugOutMeasurable: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [HarmonyEquatable]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct MeanSunModelType: HandyJSON {
...
struct CuttingEdgeModelType: HandyJSON {
