...
/*: "type" :*/
fileprivate let k_managerNameMakeValue:String = "tpoppe"

/*: "You are currently on a call, please try again later" :*/
fileprivate let kScaleFrameData:[UInt8] = [0x6d,0x5b,0x41,0x14,0x55,0x46,0x51,0x14,0x57,0x41,0x46,0x46,0x51,0x5a,0x40,0x58,0x4d,0x14,0x5b,0x5a,0x14,0x55,0x14,0x57,0x55,0x58,0x58,0x18,0x14,0x44,0x58,0x51,0x55,0x47,0x51,0x14,0x40,0x46,0x4d,0x14,0x55,0x53,0x55,0x5d,0x5a,0x14,0x58,0x55,0x40,0x51,0x46]

private func infoName(video num: UInt8) -> UInt8 {
    return num ^ 52
}

/*: "onCallResume" :*/
fileprivate let k_giftName:String = "onCaextension to add share self"
fileprivate let kTopData:String = "pin exceptionllResume"
...
// __CLOSE_PRINT__
//
//  RowReactiveCompatible+VideoChat.swift
//  AbroadTalking
//
...
extension RowReactiveCompatible {
    ...
    func situation(info: [String: Any], situation: ProvenienceChatSituation = .Start) {
        //: let json = JSON(info)
        let json = JSON(info)
        //: let type = json["type"].intValue
        let type = json[(k_managerNameMakeValue.replacingOccurrences(of: "pop", with: "y"))].intValue
...
extension RowReactiveCompatible {
    ...
    private func cheek(info: [String: Any], resumeTalk: Bool) {
        //: if !resumeTalk && TalkingSocketManager.shared.isTalking {
        if !resumeTalk && SendSocketDelegate.shared.isTalking {
            //: self.func__showStatusBarErrorMsg(showMsg: "You are currently on a call, please try again later".localized)
            self.appearShow(showMsg: String(bytes: kScaleFrameData.map{infoName(video: $0)}, encoding: .utf8)!.localized)
            //: return
            return
        }
        ...
        if belowResume()!.isKind(of: LastViewController.self) ||
            //: currentViewController()!.isKind(of: TalkingVoiceChatViewController.self) ||
            belowResume()!.isKind(of: JumpErrorDelegate.self) ||
            //: currentViewController()!.isKind(of: TalkingVideoInitivCallVC.self) {
            belowResume()!.isKind(of: VideoPlayerDelegate.self)
        {
            //: return
            return
        }
        ...
        if type == 0 { // 视频通话
            ...
            (vc as! LastViewController).chatModel = MakeChatModel.nameBy(info: info, situation: resumeTalk ? .Resume : .Recieved)
            //: } else {
        } else { // 语音通话
            //: vc = TalkingVoiceChatViewController()
            vc = JumpErrorDelegate()
            ...
            (vc as! JumpErrorDelegate).chatModel = MakeChatModel.nameBy(info: info, situation: resumeTalk ? .Resume : .Recieved)
        }

        //: let callResume = json["onCallResume"].stringValue
        let callResume = json[(String(k_giftName.prefix(4)) + String(kTopData.suffix(8)))].stringValue
        ...
        if resumeTalk, callResume.count > 0, callResume == "1" { // 恢复通话
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                self.headData(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: resumeTalk, resumeData: info)
            }
            //: } else {
        } else { // 构建音视频通话
            //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: resumeTalk, resumeData: info)
    ...
    private func headData(roomVC: UIViewController, isVideoTalk: Bool, resumeTalk: Bool, resumeData: [String: Any]) {
        //: guard var currentVc = currentViewController() else { return }
        guard var currentVc = belowResume() else { return }
        ...
        if currentVc.isKind(of: UINavigationController.self) {
            ...
            //: let rootVc = NameMacroDefine.getWindow().rootViewController as! UITabBarController
            let rootVc = NameMacroDefine.playWindow().rootViewController as! UITabBarController
        ...
        if !isVideoTalk {
            ...
            talkClass = NSStringFromClass(JumpErrorDelegate.self)
            //: } else {
        } else {
            //: talkClass = NSStringFromClass(TalkingVideoChatViewController.self)
            talkClass = NSStringFromClass(LastViewController.self)
        }
        //: var chatIndex = -1
        var chatIndex = -1
        //: var index = -1
        ...
        if isVideoTalk {
            ...
            var currentChatVc: LastViewController?
            //: let pushChatVc = roomVC as! TalkingVideoChatViewController
            let pushChatVc = roomVC as! LastViewController

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVideoChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? LastViewController
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    ...
                    currentChatVc?.imageEqual(resumeData: resumeData)
                }
                //: return
                return
            }
            ...
            if navVc!.isKind(of: IndexControllerDelegate.self) {
                //: let nav = navVc as! TalkingNavigationController
                let nav = navVc as! IndexControllerDelegate
            ...
            if topVc != nil, topVc!.isKind(of: ReverberationThen.self) {
            ...
            var currentChatVc: JumpErrorDelegate?
            //: let pushChatVc = roomVC as! TalkingVoiceChatViewController
            let pushChatVc = roomVC as! JumpErrorDelegate

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVoiceChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? JumpErrorDelegate
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    //: currentChatVc?.updateInfoViaResumeTalkData(resumeData: resumeData)
                    currentChatVc?.userData(resumeData: resumeData)
                }
                //: return
                return
            }
