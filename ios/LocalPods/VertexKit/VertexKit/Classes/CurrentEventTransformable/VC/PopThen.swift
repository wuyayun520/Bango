...
/*: "tab" :*/
fileprivate let kBottomData:String = "originab"

/*: "page" :*/
fileprivate let kShareColorData:[Character] = ["p","a","g","e"]

/*: "uid" :*/
fileprivate let kBubbleName:String = "UID"

/*: "model" :*/
fileprivate let kSizeText:[Character] = ["m","o","d","e","l"]

/*: "CompartmentView" :*/
fileprivate let k_topClickData:String = "Talkindrop list content"
fileprivate let kMakeData:String = "gMomesize to path view in"
fileprivate let k_sectionData:String = "equal index gesture view imageemCell"

/*: "There's no feed yet." :*/
fileprivate let kToName:String = "There\'s "
fileprivate let k_captureName:String = "false name tap self hiddenno fe"
...
class PopThen: UIViewController {
	var cityCount: Int = 24
	var bagMagnitude: Double = 25.9
	var bringHomeTitle: String = "constraint"
	var careDictionary: [AnyHashable: String] = [:]
    ...
    var seleteCell = CompartmentView()
    //: var lastVideoUrl = ""
    var lastVideoUrl = ""
    ...
    lazy var mainTableView: UITableView = {
        ...
        table.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: kLet_buttonData, right: 0)
        //: return table
        return table
...
extension PopThen {
    ...
    func doButton() {
        ...
        var dict = [String: Any]()
        //: dict["tab"] = 1
        dict[(kBottomData.replacingOccurrences(of: "origin", with: "t"))] = 1
        //: dict["page"] = pageIndex
        dict[(String(kShareColorData))] = pageIndex
        //: TalkingMomentRequestTool.req_MomentList(params: dict) { succeed, result, errorModel in
        BallShapedRequestTool.utility(params: dict) { succeed, result, errorModel in
...
extension PopThen {
    ...
    @objc func marginNotification(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let uid: String = userinfo["uid"] as! String
        let uid: String = userinfo[(kBubbleName.lowercased())] as! String
    ...
    @objc func like(notification: NSNotification) {
        ...
        let model: SurroundingsModelType = userinfo[(String(kSizeText))] as! SurroundingsModelType
        //: for i in 0..<self.dataSourceArr.count {
        for i in 0 ..< self.dataSourceArr.count {
    ...
    @objc func heel(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: SurroundingsModelType = userinfo[(String(kSizeText))] as! SurroundingsModelType
...
extension PopThen: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = (String(k_topClickData.prefix(6)) + String(kMakeData.prefix(5)) + "ntIt" + String(k_sectionData.suffix(6)))
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingMomentItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? CompartmentView
        //: if cell == nil {
        if cell == nil {
    ...
    func tableView(_: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if self.dataSourceArr.count > indexPath.row {
            ...
            if indexPath.row == 0 && self.isfirstvideoCell == true {
                ...
                if model.momentType == MomentClusterLiteral.Video.rawValue {
                    //: guard let tempcell: TalkingMomentItemCell = cell as? TalkingMomentItemCell  else {
                    guard let tempcell: CompartmentView = cell as? CompartmentView else {
                        //: return
                        return
                    }
                    //: tempcell.playVideo()
...
extension PopThen: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: scrollViewEndScrollPlayer(scrollView: scrollView)
        sexView(scrollView: scrollView)
    }
    ...
    func sexView(scrollView: UIScrollView) {
        ...
        if let middleIndexPath = mainTableView.indexPathForRow(at: CGPoint(x: 0, y: scrollView.contentOffset.y + mainTableView.frame.size.height / 2)) {
            ...
            if dataSourceArr.count > middleIndexPath.row {
                ...
                let cell: CompartmentView = mainTableView.cellForRow(at: middleIndexPath) as! CompartmentView
                ...
                if model.momentType == MomentClusterLiteral.Video.rawValue {
                    ...
                    if lastVideoUrl != model.videoInfo?.videoUrl {
                        // 在播放下一个视频前停止当前视频
                        //: seleteCell.stopPlay()
                        seleteCell.imagePlayPackage()
                        //: cell.stopPlay()
                        cell.imagePlayPackage()
...
extension PopThen {
    ...
    private func setupCount() {
        ...
        style.TipsTitle = (kToName + String(k_captureName.suffix(5)) + "ed yet.").localized
        ...
        mainTableView.register(CompartmentView.self, forCellReuseIdentifier: (String(k_topClickData.prefix(6)) + String(kMakeData.prefix(5)) + "ntIt" + String(k_sectionData.suffix(6))))
    ...
    private func toAction() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(like(notification:)), name: kLet_scalePathTitle, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(marginNotification(notification:)), name: kLet_statusContent, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(heel(notification:)), name: kLet_listName, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(finish), name: kLet_errNoScreenValue, object: nil)
    }
}
