...
fileprivate let k_styleBackData:[UInt8] = [0xe7,0xe0,0xe7,0xfa,0xa6,0xed,0xe1,0xea,0xeb,0xfc,0xb4,0xa7,0xae,0xe6,0xef,0xfd,0xae,0xe0,0xe1,0xfa,0xae,0xec,0xeb,0xeb,0xe0,0xae,0xe7,0xe3,0xfe,0xe2,0xeb,0xe3,0xeb,0xe0,0xfa,0xeb,0xea]

private func momentOfTruth(gift num: UInt8) -> UInt8 {
    return num ^ 142
}
...
class PuttThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_styleBackData.map{momentOfTruth(gift: $0)}, encoding: .utf8)!)
    }
    ...
    init(frame: CGRect, sliderWidth: CGFloat, sliderColor: UIColor, progressHeight: CGFloat, progressBgColor: UIColor, progressPlayedColor: UIColor, progressCachedColor: UIColor, isShowCorner: Bool) {
        ...
        //: self.setupSubviews()
        self.enableBusinessAsKey()
        //: self.setupSubViewsConstraint()
        self.positionConstraint()
    ...
    private lazy var progressView: RecordThen = {
        ...
        let v = RecordThen(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(jab(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(toUp(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension PuttThen {
    ...
    @objc func jab(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func toUp(gesture: UIPanGestureRecognizer) {
...
extension PuttThen {
    ...
    private func enableBusinessAsKey() {
    ...
    private func territorialDivision() {}
}
