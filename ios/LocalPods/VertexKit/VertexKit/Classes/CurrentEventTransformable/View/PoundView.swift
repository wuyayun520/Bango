...
fileprivate let kSearchTitle:[UInt8] = [0xc8,0xcd,0xc8,0xd3,0x87,0xc2,0xce,0xc3,0xc4,0xd1,0x99,0x88,0x7f,0xc7,0xc0,0xd2,0x7f,0xcd,0xce,0xd3,0x7f,0xc1,0xc4,0xc4,0xcd,0x7f,0xc8,0xcc,0xcf,0xcb,0xc4,0xcc,0xc4,0xcd,0xd3,0xc4,0xc3]

fileprivate func imageI(target num: UInt8) -> UInt8 {
...
class PoundView: UICollectionViewCell {
	var indexContent: String = "icon"
	var libraryArray: [AnyHashable] = []
	var refreshDictionary: [AnyHashable: String] = [:]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kSearchTitle.map{imageI(target: $0)}, encoding: .utf8)!)
    }
    ...
    func fullPhaseOfTheMoon() {
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(gildTheLilyTable))
...
extension PoundView: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func move(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func gildTheLilyTable(tap: UIGestureRecognizer) {
    ...
    func unseeable(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.followOut(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func tableImage(image: UIImage) {
