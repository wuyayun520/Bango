...
/*: "%@%@" :*/
fileprivate let k_playerValue:String = "%@%@"

/*: "/TXCache" :*/
fileprivate let kModelData:[Character] = ["/","T","X","C","a","c","h","e"]
...
protocol ServiceMakeThen: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func illegitimacy(player: PanelPlayerManager, status: TextPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func appSendTime(player: PanelPlayerManager, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func beyondGenerate(player: PanelPlayerManager, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class PanelPlayerManager: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: ServiceMakeThen?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: TextPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = PanelPlayerManager()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(kModelData))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(toActivity), userInfo: nil, repeats: true)
...
extension PanelPlayerManager {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func pushName(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: kLet_domainData.constraintView(), with: kLet_intimateData)
    ...
    @objc func toActivity() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.beyondGenerate(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func theUser(renderMode: TimeRenderMode) {
    ...
    func examineUpwardlyMedication(time: Float) {
    ...
    func moment(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isTalking == true {
        if SendSocketDelegate.shared.isTalking == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func sizeCheck() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.illegitimacy(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func buttonPhone() -> Bool {
...
extension PanelPlayerManager: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.appSendTime(player: self, duration: Int(duration), currentTime: Int(currentTime))
