...
/*: "id" :*/
fileprivate let kVoiceText:[Character] = ["i","d"]

/*: "reply" :*/
fileprivate let k_equalTitle:String = "remakey"

/*: :  :*/
fileprivate let k_toCheckViewData:String = ": "

/*:  : :*/
fileprivate let k_layerContent:String = "stop:"
...
// __CLOSE_PRINT__
//
//  FictitiousNameTransformable.swift
//  AbroadTalking
//
...
struct FictitiousNameTransformable: HandyJSON {
    //: var cid: String?
...
extension FictitiousNameTransformable {
    ...
    mutating func available() {
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if TextLanguageManager.shared.direction == .leftToRight {
                ...
                tempContent = (k_equalTitle.replacingOccurrences(of: "make", with: "pl")).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (k_equalTitle.replacingOccurrences(of: "make", with: "pl")).localized
            }
        ...
        let sizeheight = Int(GiftView.giftPostNumber(text: tempContent, size: CGSize(width: kLet_errData - 70, height: CGFloat(MAXFLOAT)), font: UIFont.ofAndSize(type: .Regular, fontSize: 16), lineNumber: 0).height)
