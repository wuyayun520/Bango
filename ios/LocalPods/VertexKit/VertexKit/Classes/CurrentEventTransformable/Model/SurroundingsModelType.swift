...
/*: "id" :*/
fileprivate let k_addName:String = "ilab"
...
struct SurroundingsModelType: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
    ...
    var pic = [UserTransformable]()
    //: var momentType: String?
    var momentType: String?
    //: var videoInfo: TalkingMomentVideoModel?
    var videoInfo: ProspectTransformable?
...
struct UserTransformable: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum MomentClusterLiteral: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct ProspectTransformable: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: LengthContentSendable?
}

///
//: enum MomentVideoStatus: String {
enum LengthContentSendable: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension SurroundingsModelType {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func streetwise(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(GiftView.giftPostNumber(text: content!, size: CGSize(width: kLet_errData - 54, height: CGFloat(MAXFLOAT)), font: UIFont.ofAndSize(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == MomentClusterLiteral.Photo.rawValue || momentType == MomentClusterLiteral.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
